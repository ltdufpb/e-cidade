...
for($x = 0; $x < pg_num_rows($result);$x++){
   ...
   if ($pdf->gety() > $pdf->h - 30 || $troca != 0 ){
      ...
      $pdf->ln(20);
      $pdf->setx(35);
      $pdf->cell(100,5,strtoupper((string) $munic).'-'.strtoupper((string) $uf).', '.date('d').' de '.db_mes(date('m')).' de '.date('Y').'.',0,1,"L",0);
