...
{
	static $aColors=['transparent','black','white','red','green','blue','cyan','magenta',
		'yellow','dkGray','gray','ltGray'];

	if(str_starts_with((string) $color, '#'))
	{
		return sprintf("['RGB',%.3f,%.3f,%.3f]", hexdec(substr((string) $color,1,2))/255,
			hexdec(substr((string) $color,3,2))/255, hexdec(substr((string) $color,5,2))/255);
	}
...
function TextField($name,$w,$h,$prop=[])
{
...
function ComboBox($name,$w,$h,$values,$prop=[])
{
...
function CheckBox($name,$w,$checked=false,$prop=[])
{
...
function Button($name,$w,$h,$caption,$action,$prop=[])
{
	if(!isset($prop['BorderColor']))
		$prop['BorderColor']='black';
	$prop['BorderStyle']='beveled';
	$this->_addfield('button',$name,$this->x,$this->y,$w,$h,$prop);
	$this->script.="f.buttonSetCaption('".addslashes((string) $caption)."');";
	$this->script.="f.setAction('MouseUp','".addslashes((string) $action)."');";
