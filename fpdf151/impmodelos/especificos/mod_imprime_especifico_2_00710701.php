...
for ($i = 0;$i < 2;$i++){
  ...
  for($ii = 0;$ii < $this->linhasdadospagto;$ii++) {
    ...
    $this->objpdf->cell(9,3,"(".trim(pg_fetch_result($this->recorddadospagto,$ii,$this->receitared)).")",0,0,"R",0);
    ...
    if ($valor < 0 || $valor == null){
         $this->totaldesc += pg_fetch_result($this->recorddadospagto,$ii,$this->valor) + $this->descontoLICBomPagador->j21_valor;
    }else if ($codtipo == 't' and $valor > 0){
         $this->totalacres  += pg_fetch_result($this->recorddadospagto,$ii,$this->valor);
    }else{
      if(!$somouLICBomPagador) {
        $this->totalrec  += pg_fetch_result($this->recorddadospagto,$ii,$this->valor) + abs($this->descontoLICBomPagador->j21_valor);
      } else {
    ...
    if(!$somouLICBomPagador && pg_fetch_result($this->recorddadospagto,$ii,"k00_hist") != 918) {
      $valores = pg_fetch_result($this->recorddadospagto,$ii,$this->valor) + abs($this->descontoLICBomPagador->j21_valor);
      $somouLICBomPagador = true;
    } else if(!$existeLinhaDesconto && pg_fetch_result($this->recorddadospagto,$ii,"k00_hist") == 918) {
      $valores = pg_fetch_result($this->recorddadospagto,$ii,$this->valor) + $this->descontoLICBomPagador->j21_valor;
  ...
  //IF QUE ESCREVE ÚLTIMA LINHA DE DESCONTO CASO NÃO TENHA SIDO REGISTRADO EM OUTRA LINHA
    if(!$existeLinhaDesconto && ($ii+1 == $this->linhasdadospagto)) {
      if($this->descontoLICBomPagador) {
        ...
        $this->objpdf->setx($xcol+3+$maiscol);
        $this->objpdf->cell(7,3,trim((string) $this->descontoLICBomPagador->j21_receit),0,0,"R",0);
        $this->objpdf->cell(9,3,"(".trim(pg_fetch_result($this->recorddadospagto,0,$this->receitared)).")",0,0,"R",0);
        $this->objpdf->cell(63,3, trim(pg_fetch_result($this->recorddadospagto,0,$this->ddreceita)." (desconto)"),0,0,"L",0);
