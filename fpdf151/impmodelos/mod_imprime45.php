...
for ($i = 0; $i < 2; $i++) {
    ...
    if (!empty($this->intermediadorCpf)) {
        ...
        if (strlen((string) $this->intermediadorCpf) == 11) {
            $sDocumento = db_cgccpf($this->intermediadorCpf);
        } elseif (strlen((string) $this->intermediadorCpf) == 14) {
            $sDocumento = db_cgccpf($this->intermediadorCpf);
        }
    ...
    if ($this->tipoitbi == "urbano") {
        ...
        $this->objpdf->cell(40, $altura, (count($this->areaterrenomat) == 1 ? db_formatar($this->areatran, 'f', ' ', ' ', ' ', 6) . ($this->tipoitbi == "urbano" ? 'm2' : 'ha') : (strlen((string) $this->areaterrenomat[1]) > 2 ? $this->areatran : db_formatar($this->areatran, 'f', ' ', ' ', ' ', 6) . ($this->tipoitbi == "urbano" ? 'm2' : 'ha'))), 1, 1, "R", 0);
        $this->objpdf->setx(100);
        $this->objpdf->SetFont('Arial', 'B', 8);
        ...
        $this->objpdf->cell(40, $altura, (@$this->areatotal == 0 ? '' : (count(@$this->areaedificadamat) == 1 ? db_formatar(@$this->areatrans, 'f', ' ', ' ', ' ', 6) . 'm2' : (strlen((string) @$this->areaedificadamat[1]) > 2 ? db_formatar(@$this->areatrans, 'f', ' ', ' ', ' ', 6) . 'm2' : db_formatar(@$this->areatrans, 'f', ' ', ' ', ' ', 6) . 'm2'))), 1, 1, "R", 0);
        ...
        $this->objpdf->cell(22, $altura, 'Logradouro:', 1, 0, "L", $iPreencheFundo);
        $this->objpdf->SetFont('Arial', '', 8);
        $this->objpdf->cell(66, $altura, substr((string) $this->j14_nome, 0, 40), 1, 1, "L", 0);

        $lengthLocalizacao = strlen((string) $this->it18_localimovel);
        ...
        $this->objpdf->cell(41, $altura, (count($this->areaterrenomat) == 1 ? db_formatar($this->areatran, 'f', ' ', ' ', ' ', 6) . $unMedida : (strlen((string) $this->areaterrenomat[1]) > 2 ? $this->areatran . $unMedida : db_formatar($this->areatran, 'f', ' ', ' ', ' ', 6) . $unMedida)), 1, 1, "R", 0);
        $this->objpdf->setx(100);
        $this->objpdf->SetFont('Arial', 'B', 8);
        $this->objpdf->cell(24, $altura, 'Benfeitorias:', 1, 0, "L", $iPreencheFundo);
        $this->objpdf->SetFont('Arial', '', 8);
        $this->objpdf->cell(41, $altura, (@$this->areatotal == 0 ? '' : (count(@$this->areaedificadamat) == 1 ? db_formatar(@$this->areatotal, 'f', ' ', ' ', ' ', 6) . 'm2' : (strlen((string) @$this->areaedificadamat[1]) > 2 ? db_formatar(@$this->areatotal, 'f', ' ', ' ', ' ', 6) . 'm2' : db_formatar(@$this->areatotal, 'f', ' ', ' ', ' ', 6) . 'm2'))), 1, 0, "R", 0);
    ...
    if (count($this->aDadosFormasPgto) > 0) {
        foreach ($this->aDadosFormasPgto as $iInd => $aDadosFormas) {
            if ($iInd <= 2) {
                ...
                $this->objpdf->cell(20, $altura, $aDadosFormas['Aliquota'] . (trim((string) $aDadosFormas['Aliquota']) != "" ? "%" : ""), 1, 0, "R", 0);
