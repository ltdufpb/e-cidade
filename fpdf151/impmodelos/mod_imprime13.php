...
if (!function_exists('truncar')) {
    function truncar($valor)
    {
        ...
        $novoValor = explode(".", (string) $novoValor);
        if (sizeof($novoValor) > 1) {
            if (strlen((string) $novoValor[1]) > 2) {
                $novoValor[1] = substr((string) $novoValor[1], 0, 2);
...
if(isset($this->Sdata) && trim((string) $this->Sdata)!=""){
...
$this->objpdf->multicell(175,4,stripslashes((string) $Sresumo),0,"J");
...
$sCpfCnpj = (trim((string) $this->cnpj)!=""?
                 (strlen((string) $this->cnpj) == 11?db_formatar($this->cnpj,'cpf'):db_formatar($this->cnpj,'cnpj')):"");
...
$maiscol = 0;
$this->objpdf->SetWidths([12,15,19,69,29,23,12]);
$this->objpdf->SetAligns(['C','C','C','J','J','J','L','R']);
...
//     $dvalidademinima = pg_result($this->recorddositens,$ii,$this->Dvalidademinima);
     ...
     $dist = 4;
     if(trim((string) $codunid)!=""){
         $unid = trim(substr((string) $unid,0,10));
         if($susaquant=="t"){
             $unid  .= " \n$quantunid UNIDADES\n";
             //$resumo = str_replace("\n","",$resumo);
         }
