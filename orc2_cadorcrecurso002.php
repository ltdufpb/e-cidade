...
if (isset($oGet->recurso) && $oGet->recurso != "") {
	 if ($oGet->recurso == 't') {
         ...
         if ($oGet->recurso == 'sa') {
               ...
               $dbwhere = " o15_datalimite is not null or o15_datalimite < '{$date}'";
         }
     }
}

$sSql = $clorctiporec->sql_queryComplemento(null,"*","o15_recurso", $dbwhere);
$result  = $clorctiporec->sql_record($sSql);
...
for ($x = 0; $x < pg_numrows($result); $x++) {
    ...
    if ($pdf->gety() > $pdf->h - 30 || $troca != 0) {
        $pdf->addpage();
        $pdf->setfont('arial', 'b', 8);
        $pdf->cell(84, $alt, "Fonte de Recurso", 1, 0, "C", 1);
        $pdf->cell(84, $alt, "Complemento", 1, 0, "C", 1);
