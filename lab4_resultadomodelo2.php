...
try {
    foreach ($oDadosEstrutura->aSetor as $iSetor => $oSetor) {
        ...
        foreach ($oSetor->aExames as $oExame) {
            imprimirAtributosExame($oPdf, $oExame, $oDadosEstrutura);
...
function imprimirAtributosExame($oPdf, $oExame, $oDadosEstrutura)
{
...
function validaQuebraPagina($oPdf, $oAtributo)
{
    ...
    if (!empty($oAtributo->valorabsolutoanterior)) {
        $iNumeroLinhasComplemento += $oPdf->nbLines(187,
 ...
 */
function montaCabecalho($oPdf, $departamento)
{
    $oPdf->AddPage();
