...
if(isset($sDatai) && isset($sDataf)) {
  ...
  if($sDatai!="" && $sDataf!="") {
    ...
    $sRest     = "";
    $sRest     = substr((string) $sDatai, 6);
    $sRest    .="-";
    $sRest    .= substr((string) $sDatai, 3, 2);
    $sRest    .="-";
    $sRest    .= substr((string) $sDatai, 0, 2);
    $sWhere   .= "sd23_d_consulta  BETWEEN '".$sRest."' and";
    $sRest     = "";
    $sRest     = substr((string) $sDataf, 6);
    $sRest    .="-";
    $sRest    .= substr((string) $sDataf, 3, 2);
    $sRest    .="-";
    $sRest    .= substr((string) $sDataf, 0, 2);
    $sWhere   .= " '".$sRest."'";
  }
}
...
$oPdf->Open();
$oPdf->AliasNbPages();
$oPdf->SetWidths( [ 16, 10, 20, 9, 16, 12, 12, 55, 41 ] );
$oPdf->SetAligns( [ "C", "C", "C", "C", "C", "C", "C", "L", "C" ] );
...
for ($iX = 0; $iX < $oClagendamentos->numrows; $iX++) {
  ...
  $sData_Consulta = "";
  $sData_Consulta = substr((string) $sd23_d_consulta,8,2);
  $sData_Consulta .="/";
  $sData_Consulta .= substr((string) $sd23_d_consulta, 5,2 );
  $sData_Consulta .="/";
  $sData_Consulta .= substr((string) $sd23_d_consulta, 0, 4);
