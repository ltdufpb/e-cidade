 ...
 */
 ...
 if($limite==0){
  ...
  $num=$resultsql === false || $resultsql === null ? 0 : pg_num_rows($resultsql);  
if($resultsql==false){
  ...
  $head6 = "";
  $head7 = "";
  $head8 = substr((string) $finalidade, 0,30);
  $head9 = substr((string) $finalidade,30,30);
 ...
 $DB_instit = db_getsession("DB_instit");
  ...
  for ($xi=0;$xi<pg_num_rows($resultsql);$xi++){
    ...
    for ($c=0;$c<$fm_numfields;$c++){
      ...
      $cor=preg_split("#\\##m", (string) ${$corpcortexto});
      $pdf->SetTextColor(@$cor[0],@$cor[1],@$cor[2]);
      $corpcorborda="corpcorborda_".$c;     
      $cor=preg_split("#\\##m", (string) ${$corpcorborda});
      $pdf->SetDrawColor(@$cor[0],@$cor[1],@$cor[2]);
    ...
    if(${$corpcorfundo} =="255#255#255"){
      if($xi % 2!=0){
       $cor=preg_split("#\\##m",(string) $intercalar1);
       $pdf->SetFillColor(@$cor[0],@$cor[1],@$cor[2]);
      }else{
       $cor=preg_split("#\\##m",(string) $intercalar2);
       $pdf->SetFillColor(@$cor[0],@$cor[1],@$cor[2]);
      }	 
    }else{
          $corpcorfundo="corpcorfundo_".$c;     
          $cor=preg_split("#\\##m",(string) ${$corpcorfundo});
          $pdf->SetFillColor(@$cor[0],@$cor[1],@$cor[2]);
    }
	 ...
	 $quebra=1;
      ...
      $pdf->Cell(${$corplargura},${$corpaltura},pg_fetch_result($resultsql,$xi,$c),1,$quebra,"",1);
	    ...
	    $quebra=0;
	     ...
	     $quebra=1;
          ...
          switch(${$corpopcao}){
                 ...
                 case "somar":
                        for($d=0; $d< $num ; $d++){
                          $result=pg_fetch_result($resultsql, $d, $cc);
                          $soma+=$result;
                        }
