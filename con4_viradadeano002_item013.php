...
if ($sqlerro == false) {
    ...
    if (($linhasorigem > 0) && ($linhasdestino == 0)) {
        ...
        // vamos alterar para nao pegar recurso da dotacao e sim da origem do empenho
       // $sqlemp .= "         o58_codigo, ";
       $sqlemp .= " (select o206_recurso from origemcomplementorecurso where o206_numero = e60_numemp and o206_origem = 1) as o58_codigo, ";
        ...
        for ($e = 0; $e < $linhasemp; $e++) {
            ...
            if ($linhasemp2 == 0) {
                ...
                if ($iTipo == "999") {
                    ...
                    $cldb_viradaitemlog->c35_log = "Empenho [{$e60_numemp}] sem tipo para Codcon: [{$e64_codele}] processado utilizando o tipo 999 ";
        ...
        if ($sqlerro == false) {
            ...
            for ($iInstituicao = 0; $iInstituicao < pg_num_rows($rsInstituicoes); $iInstituicao++) {
                ...
                $sSqlEmpenho999 .= " where e91_anousu  = {$anodestino}                                  ";
                ...
                for ($iEmpenho999 = 0; $iEmpenho999 < pg_num_rows($rsEmpenho999); $iEmpenho999++) {
                    ...
                    foreach ($aBuscaTransacao as $oStdTransacao) {
                        if (comparaElementoDespesa($oEmpenho999->c60_estrut, $oStdTransacao->c114_elemento)) {
                            //emprestotipo
                            $sSqlEmprestotipo = "select *
                                     from emprestotipo
                                    where '$oStdTransacao->estrutural_emprestotipo' ilike e90_estrut||'%'
                                    order by e90_estrut desc limit 1";
                            ...
                            $sUpdate = "update empresto
                                 set e91_codtipo = {$oEmprestotipo->e90_codigo}
                               where e91_anousu  = {$oEmpenho999->e91_anousu}
                                 and e91_numemp  = {$oEmpenho999->e91_numemp}";
...
function getEmpRestoTipo( $oDadosRecursivo)
{
    ...
    $sSql = $oDaoContrans->sql_queryVinculoEmpRestoTipoLiquidacao(null,
                                                                  "c60_estrut",
                                                                  null,
                                                                  "c114_elemento = '{$sElemento}'
                                                                   and c45_anousu = {$iAnoUsu}");
...
function getCodigoTipo($e64_codele, $iAnoBusca){

  /**
   * Alteramos a regra para buscar o tipo na emprestotipo
   * quando nao encontrar na query acima
   *
  */
  $iTipo = 999; // agora se nao encontrar segue o 999
  $oDaoOrcElemento = new cl_orcelemento;
  $sSqlElemento = $oDaoOrcElemento->sql_query ( $e64_codele, $iAnoBusca, "o56_elemento", null, "");
  ...
  if ( $oDaoOrcElemento->numrows > 0 ) {
      ...
      $oDadosRecursivo->iPosicao = 15;
      $sElementoVinculo = getEmpRestoTipo($oDadosRecursivo);
      if ($sElementoVinculo == null) {
        return $iTipo;
      }
      // com o elemento do vinculo do estrutural buscamos o tipo dele na emprestotipo
      $oDaoEmprestoTipo = new cl_emprestotipo;
      $sSqlTipo =  $oDaoEmprestoTipo->sql_query (null,
                                                 "e90_codigo",
                                                 null,
                                                 "rpad(e90_estrut, 15, '0') = '{$sElementoVinculo}'");
      $rsTipo = $oDaoEmprestoTipo->sql_record($sSqlTipo);

      if ($oDaoEmprestoTipo->numrows > 0) {
        $iTipo = db_utils::fieldsMemory($rsTipo, 0)->e90_codigo;
      }
  }
  return $iTipo;
}
