...
if (!empty($iDataEmissao)){
  ...
  $dEmissao = $data_emiExp[2]."-".$data_emiExp[1]."-".$data_emiExp[0];
}
...
for ( $ww = 0; $ww < $clmatricula->numrows; $ww ++ ) {
    ...
    $ed47_v_nome = is_null($ed47_v_nomesocial) || empty($ed47_v_nomesocial) ? $ed47_v_nome : $ed47_v_nomesocial;
    $dataEmissao  = !empty($iDataEmissao) ? strtotime($dEmissao) : DB_getsession("DB_datausu") ;
    ...
    $pdf->setfont('arial', '', 7);
    $pdf->cell(35, 4, strip_tags((string) $Led47_tiposanguineo), 0, 0, "L", 0);
    ...
    $pdf->setfont('arial', '', 7);
    $pdf->cell(35, 4, "Tipo filiação:", 0, 0, "L", 0);
    ...
    if ($clmatricula->numrows > 0) {
        ...
        $pdf->setfont('arial', 'b', 7);
        $pdf->cell(85, 4, $ed18_c_abrev, "RT", 1, "L", 1);
    ...
    for ($r = 0; $r < $linhas0; $r++) {
        ...
        if ($parecer != "") {
            ...
            $pdf->setfont('arial', '', 7);
            $pdf->multicell(190, 3, "  " . trim((string) $parecer), "LRB", "J", 0, 0);
        }
    ...
    for ($e = 0; $e < $cldiarioresultado->numrows; $e++) {
        ...
        if ($ed73_t_parecer != "") {
            ...
            $pdf->setfont('arial', '', 7);
            $pdf->multicell(190, 3, "  " . trim((string) $ed73_t_parecer), "LRB", "J", 0, 0);
            $pdf->cell(190, 4, "", 0, 1, "C", 0);
        }
    }
     ...
     */
    $sResultadoFinal = ResultadoFinal($ed60_i_codigo, $ed60_i_aluno, $ed60_i_turma, trim((string) $ed60_c_situacao), trim((string) $ed60_c_concluida), $ensino);
    $pdf->setfont('arial', 'b', 7);
    ...
    if ($obs1 != "") {
        ...
        $pdf->setfont('arial', '', 7);
        $pdf->multicell(190, 4, $obs1, 1, "L", 0, 0);
    }
...
function imprimirAssinaturas($pdf, $oGet, $oTurma)
{
    ...
    foreach ($assinaturas as $index => $dados) {
        ...
        $pdf->setX($eixoX);
        $pdf->cell(45, 3, $dados->cargo, 0, 1, "L", 0);
