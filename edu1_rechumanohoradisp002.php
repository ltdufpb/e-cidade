...
if ($oPost->sAction == 'PesquisaPeriodo') {
  ...
  $restricao .= "                 join rechumanoativ on rechumanoativ.ed22_i_codigo = rechumanohoradisp.ed33_rechumanoativ ";
  ...
  $restricao .= "               and  ed22_datafim is null";
  $restricao .= "              )";
    ...
    for ( $iContador = 0; $iContador < count($aPeriodosInformados); $iContador++ ) {
      ...
      foreach ( $aAgendaProfissionalValida as $oAgendaProfissional ) {
        ...
        if ( ($iHoraInicioPeriodo >= $iHoraInicioAgenda) && ( $iHoraFimPeriodo <= $iHoraFimAgenda) ) {
          ...
          $sWhereConflito  .= "  AND  ed129_diasemana = {$oAgendaProfissional->getDiaSemana()} and ed129_turno = {$oPeriodoEscola->getTurno()->getCodigoTurno()}";
          $sWhereConflito  .= "  AND  ed129_horainicio = '{$oPeriodoEscola->getHoraInicio()}'";
          ...
          $sWhereConflito  .= " AND ed22_datafim is null";
          $sSqlConflito     = $clrechumanohoradisp->sql_query_disponibilidade( "", $sCamposConflito, "", $sWhereConflito, true, true);
