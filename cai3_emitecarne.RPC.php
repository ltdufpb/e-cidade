    ...
    try {
      ...
      if ($iRowsRegraEmissao > 0) {
        ...
        foreach ($aRegrasEmissao as $iIndiceRegra => $oRegraEmissao) {
          ...
          foreach ($aChecks as $iIndice => $aValores) {
            ...
            if ( ($aValores["Numpar"] >= $oRegraEmissao->k48_parcini) && ($aValores["Numpar"] <= $oRegraEmissao->k48_parcfim) ) {
               ...
               if ( ($lCobrancaRegistrada || empty($sGeraCarne)) && ($oRegraEmissao->k48_cadtipomod == $iTipoModeloRecibo ||
                    ($oRegraEmissao->k48_cadtipomod == $iTipoModeloCarne && !$unica && $custasHonorarios)) ) {
                  /**
          ...
          if ( (!in_array(0, $iDiferenca) && ($lCobrancaRegistrada || $sGeraCarne == "") && $oParam->lNovoRecibo && $lEmissaoRecibo) || ($sGeraCarne == "" && !$lConfReemissaoRecibo ) ) {
...
function recibodesconto1($numpre, $numpar, $tipo, $tipo_debito, $whereloteador, $totalregistrospassados, $totregistros, $ver_matric, $ver_inscr) {
