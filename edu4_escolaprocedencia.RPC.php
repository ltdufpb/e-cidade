...
try {
  ...
  switch ($oParam->exec) {
      ...
      if (pg_num_rows($rsValidaVinculo) > 0) {
	    ...
	    $oRetorno->message = urlencode( (string) _M( CAMINHO_MENSAGENS . "escola_procedencia_excluida" ) );
      ...
      if ( empty($sNome) ) {
	    ...
	    $oRetorno->message = urlencode( (string) _M( CAMINHO_MENSAGENS . "escola_procedencia_sucesso" ) );
	    break;
  }
