...
class LanguageConv {
	...
	if( LANGUAGE_GREEK ) {
		...
		$unistring = $this->gr_win2uni($aTxt); 
	    } else  {
		$unistring = $this->gr_iso2uni($aTxt);
	    }
	    return $unistring;
	} elseif( LANGUAGE_CYRILLIC ) {
	    if( CYRILLIC_FROM_WINDOWS && (!defined('CYRILLIC_LANGUAGE_CHARSET') || stristr((string) CYRILLIC_LANGUAGE_CHARSET, 'windows-1251')) ) {
		$aTxt = convert_cyr_string($aTxt, "w", "k"); 
	    }
	    if( !defined('CYRILLIC_LANGUAGE_CHARSET') || stristr((string) CYRILLIC_LANGUAGE_CHARSET, 'koi8-r') || stristr((string) CYRILLIC_LANGUAGE_CHARSET, 'windows-1251')) {
...
//('Usage of FF_CHINESE (FF_BIG5) font family requires that your PHP setup has the iconv() function. By default this is not compiled into PHP (needs the "--width-iconv" when configured).');
	...
	$uniline='';
	for ($i=0; $i < strlen((string) $isoline); $i++){
	    $thischar = substr((string) $isoline,$i,1);
	...
	$uniline='';
	$n = strlen((string) $winline);
	for ($i=0; $i < $n; $i++) {
	    $thischar=substr((string) $winline,$i,1);
	    $charcode=ord($thischar);
...
// CONSTRUCTOR
	...
	$this->style_names=[FS_NORMAL	=>'normal',
				 FS_BOLD	=>'bold',
				 FS_ITALIC	=>'italic',
	...
	$this->font_files=[
	    ...
	    /* Chinese fonts */
	    FF_SIMSUN 	=>  [FS_NORMAL	=>'simsun.ttc',  
				  FS_BOLD	=>'simhei.ttf',   
				  FS_ITALIC	=>'',   
				  FS_BOLDITALIC	=>'' ],
	    FF_CHINESE 	=>  [FS_NORMAL	=>CHINESE_TTF_FONT, 
				  FS_BOLD	=>'', 
				  FS_ITALIC	=>'', 
				  FS_BOLDITALIC	=>'' ],

	    /* Japanese fonts */
 	    FF_MINCHO 	=>  [FS_NORMAL	=>MINCHO_TTF_FONT,  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',   
				  FS_BOLDITALIC	=>'' ],
 	    FF_PMINCHO 	=>  [FS_NORMAL	=>PMINCHO_TTF_FONT,  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',  
				  FS_BOLDITALIC	=>'' ],    
 	    FF_GOTHIC  	=>   [FS_NORMAL	=>GOTHIC_TTF_FONT,  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',   
				  FS_BOLDITALIC	=>'' ],    
 	    FF_PGOTHIC 	=>  [FS_NORMAL	=>PGOTHIC_TTF_FONT,  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',  
				  FS_BOLDITALIC	=>'' ],    
 	    FF_MINCHO 	=>  [FS_NORMAL	=>PMINCHO_TTF_FONT,  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',  
				  FS_BOLDITALIC	=>'' ],   

	    /* Hebrew fonts */
	    FF_DAVID 	=>  [FS_NORMAL	=>'DAVIDNEW.TTF',  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',  
				  FS_BOLDITALIC	=>'' ],   

	    FF_MIRIAM 	=>  [FS_NORMAL	=>'MRIAMY.TTF',  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',  
				  FS_BOLDITALIC	=>'' ],   

	    FF_AHRON 	=>  [FS_NORMAL	=>'ahronbd.ttf',  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',  
				  FS_BOLDITALIC	=>'' ],   

	    /* Dejavu fonts */
 	    FF_DV_SANSSERIF => [FS_NORMAL	=>'DejaVuSans.ttf',  
				  FS_BOLD	=>'DejaVuSans-Bold.ttf',   
				  FS_ITALIC	=>'DejaVuSans-Oblique.ttf',   
				  FS_BOLDITALIC	=>'DejaVuSans-BoldOblique.ttf' ],  

 	    FF_DV_SANSSERIFMONO => [FS_NORMAL	=>'DejaVuSansMono.ttf',  
				  FS_BOLD	=>'DejaVuSansMono-Bold.ttf',   
				  FS_ITALIC	=>'DejaVuSansMono-Oblique.ttf',   
				  FS_BOLDITALIC	=>'DejaVuSansMono-BoldOblique.ttf' ],  

 	    FF_DV_SANSSERIFCOND => [FS_NORMAL	=>'DejaVuSansCondensed.ttf',  
	    ...
	    /* Misc fonts */
 	    FF_DIGITAL =>   [FS_NORMAL	=>'DIGIRU__.TTF',  
				  FS_BOLD	=>'Digirtu_.ttf', 
				  FS_ITALIC	=>'Digir___.ttf', 
				  FS_BOLDITALIC	=>'DIGIRT__.TTF' ],   
 	    FF_SPEEDO =>    [FS_NORMAL	=>'Speedo.ttf',  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',   
				  FS_BOLDITALIC	=>'' ],   
 	    FF_COMPUTER  =>  [FS_NORMAL	=>'COMPUTER.TTF',  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',   
				  FS_BOLDITALIC	=>'' ],   
 	    FF_CALCULATOR =>[FS_NORMAL	=>'Triad_xs.ttf',  
				  FS_BOLD	=>'',   
				  FS_ITALIC	=>'',   
				  FS_BOLDITALIC	=>'' ],   
	    ];
...
// PUBLIC METHODS	
	...
	if( $family == FF_HANDWRT || $family==FF_BOOK ) {
	    (new JpGraphError())->RaiseL(25045);//('Font families FF_HANDWRT and FF_BOOK are no longer available due to copyright problem with these fonts. Fonts can no longer be distributed with JpGraph. Please download fonts from http://corefonts.sourceforge.net/');
	}

	$fam = @$this->font_files[$family];
	if( !$fam ) {
	    (new JpGraphError())->RaiseL(25046, $family);//("Specified TTF font family (id=$family) is unknown or does not exist. Please note that TTF fonts are not distributed with JpGraph for copyright reasons. You can find the MS TTF WEB-fonts (arial, courier etc) for download at http://corefonts.sourceforge.net/");
	}
	$f = @$fam[$style];

	if( $f==='' )
	    (new JpGraphError())->RaiseL(25047, $this->style_names[$style], $this->font_files[$family][FS_NORMAL]);//('Style "'.$this->style_names[$style].'" is not available for font family '.$this->font_files[$family][FS_NORMAL].'.');
	if( !$f ) {
	    (new JpGraphError())->RaiseL(25048, $fam);//("Unknown font style specification [$fam].");
	}
	...
	if( file_exists($f) === false || is_readable($f) === false ) {
	    (new JpGraphError())->RaiseL(25049, $f);//("Font file \"$f\" is not readable or does not exist.");
	}
	return $f;
    }
} // Class
