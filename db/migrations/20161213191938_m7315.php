...
class M7315 extends PostgresMigration
{
  ...
  {
    $this->tabela_db_sysarquivo   = $this->table('db_sysarquivo',    ['schema'=>'configuracoes']);
    $this->tabela_db_sysarqmod    = $this->table('db_sysarqmod',     ['schema'=>'configuracoes']);
    $this->tabela_db_syscampo     = $this->table('db_syscampo',      ['schema'=>'configuracoes']);
    $this->tabela_db_syscampodef  = $this->table('db_syscampodef',   ['schema'=>'configuracoes']);
    $this->tabela_db_sysarqcamp   = $this->table('db_sysarqcamp',    ['schema'=>'configuracoes']);
    $this->tabela_db_sysprikey    = $this->table('db_sysprikey',     ['schema'=>'configuracoes']);
    $this->tabela_db_sysforkey    = $this->table('db_sysforkey',     ['schema'=>'configuracoes']);
    $this->tabela_db_syssequencia = $this->table('db_syssequencia',  ['schema'=>'configuracoes']);
    $this->tabela_db_sysindices   = $this->table('db_sysindices',    ['schema'=>'configuracoes']);
    $this->tabela_db_syscadind    = $this->table('db_syscadind',     ['schema'=>'configuracoes']);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4001,'tipoassedb_depart','Tabela de vínculo entre departamento e tipo de assentamento.','rh184','2016-12-13','Vínculo entre departamento e tipo de assentamento',0,'f','f','t','t']
    ]);

    $this->tabela_db_sysarqmod->insert(['codmod','codarq'], [
      [29,4001]
    ]);

    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'],[
      [22213,'rh184_sequencial','int4','Código sequencial da tabela.','0','Código',19,'f','f','f',1,'text','Código'],
      [22214,'rh184_db_depart','int4','Código do departamento','0','Departamento',19,'f','f','f',1,'text','Departamento'],
      [22215,'rh184_tipoasse','int4','Código do Tipo de assentamento.','0','Tipo de Assentamento',19,'f','f','f',1,'text','Tipo de Assentamento']
    ]);

    $this->tabela_db_sysarqcamp->insert(['codarq','codcam','seqarq','codsequencia'], [
      [4001, 22213, 1, 0],
      [4001, 22214, 2, 0],
      [4001, 22215, 3, 0]
    ]);

    $this->tabela_db_sysprikey->insert(['codarq','codcam','sequen','camiden'], [
      [4001, 22213, 1, 22213]
    ]);

    $this->tabela_db_sysforkey->insert(['codarq', 'codcam', 'sequen', 'referen', 'tipoobjrel'], [
      [4001, 22214, 1, 154, 0],
      [4001, 22215, 1, 596, 0]
    ]);

    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000629,'tipoassedb_depart_rh184_sequencial_seq',1,1,9223372036854775807,1,1]
    ]);
    $this->execute('update db_sysarqcamp set codsequencia = 1000629 where codarq = 4001 and codcam = 22213');

    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4396, 'tipoassedb_depart_db_depart_tipoasse_in', 4001, '0']
    ]);

    $this->tabela_db_syscadind->insert(['codind', 'codcam', 'sequen'], [
      [4396, 22214, 1],
      [4396, 22215, 2]
    ]);
  }
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4002, 'db_departrhlocaltrab', 'Tabela que vincula departamento ao local de trabalho.', 'rh185', '2016-12-13', 'Vincula entre departamento ao local de trabalho', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22216,'rh185_sequencial','int4','Código sequencial da tabela.','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22217,'rh185_db_depart','int4','Vínculo com departamento','0', 'Departamento',19,'f','f','f',1,'text','Departamento'],
      [22218,'rh185_rhlocaltrab','int4','Vínculo com local de trabalho','0', 'Local de Trabalho',19,'f','f','f',1,'text','Local de Trabalho'],
      [22219,'rh185_instit','int4','Vínculo com a instituição','0', 'Instituição',19,'f','f','f',1,'text','Instituição']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000630, 'db_departrhlocaltrab_rh185_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    $this->execute("update db_sysarqcamp set codsequencia = 1000630 where codarq = 4002 and codcam = 22216");

    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4398,'db_departrhlocaltrab_db_depart_rhlocaltrab_instit_in',4002,'0']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4003, 'configuracoesdatasefetividade', 'Tabela que configura a efetividade', 'rh186', '2016-12-13', 'Configuração da efetividade', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22221,'rh186_exercicio',                 'int4',       'Exercício da efetividade',           '0',    'Exercício',                   19, 't', 'f', 'f', 1, 'text', 'Exercício'],
      [22222,'rh186_competencia',               'varchar(2)', 'Competência da configuração',         '',    'Competência',                 2,  't', 't', 'f', 0, 'text', 'Competência'],
      [22223,'rh186_datainicioefetividade',     'date',       'Data de Início da Efetividade',      'null', 'Data Início Efetividade',     10, 't', 'f', 'f', 1, 'text', 'Data Início Efetividade'],
      [22224,'rh186_datafechamentoefetividade', 'date',       'Data de fechamento da efetividade.', 'null', 'Data Fechamento Efetividade', 10, 't', 'f', 'f', 1, 'text', 'Data Fechamento Efetividade'],
      [22225,'rh186_dataentregaefetividade',    'date',       'Data de entrega da efetividade.',    'null', 'Data Entrega Efetividade',    10, 't', 'f', 'f', 1, 'text', 'Data Entrega Efetividade'],
      [22226,'rh186_processado',                'bool',       'Processado',                         'f',    'Processado',                   1, 'f', 'f', 'f', 5, 'text', 'Processado'],
      [22253,'rh186_instituicao',               'int4',       'Instituição',                        '0',    'Instituição',                 19, 'f', 'f', 'f', 1, 'text', 'Instituição']
    ]);
    ...
    $this->tabela_db_syscampodef->insert(['codcam', 'defcampo', 'defdescr'], [
      [22226,'f','']
    ]);
  }
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4004, 'tiporegistro', 'Tabela com os tipos de registro', 'rh187', '2016-12-13', 'Tipo de registro', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22227,'rh187_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22228,'rh187_descricao','varchar(50)','Descrição do tipo de registro','', 'Descrição',50,'f','t','f',0,'text','Descrição']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000631, 'tiporegistro_rh187_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4005, 'jornada', 'Tabela com as jornadas de trabalho dos servidores', 'rh188', '2016-12-13', 'Jornadas de trabalho', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22229,'rh188_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22230,'rh188_descricao','varchar(50)','Descrição das jornadas de trabalho','', 'Descrição',50,'f','t','f',0,'text','Descrição'],
      [22231,'rh188_fixo','bool','Se a jornada é fixa','f', 'Fixo',1,'f','f','f',5,'text','Fixo'],
      [22344,'rh188_tipo','char(1)','Informa se a jornada é F-Folga, D-DSR ou T-Trabalhado','', 'Tipo',1,'t','t','f',0,'text','Tipo']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000632, 'jornada_rh188_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4006, 'jornadahoras', 'Horas da jornada de trabalho', 'rh189', '2016-12-13', 'Jornada de trabalho', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22232,'rh189_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22233,'rh189_jornada','int4','Vínculo com a tabela de jornadas','0', 'Jornada',19,'f','f','f',1,'text','Jornada'],
      [22235,'rh189_tiporegistro','int4','Tipo de registro computado','0', 'Tipo de Registro',19,'f','f','f',1,'text','Tipo de Registro'],
      [22236,'rh189_hora','varchar(19)','Hora registrada','', 'Hora',19,'f','t','f',0,'text','Hora']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000633, 'jornadahoras_rh189_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    $this->execute("update db_sysarqcamp set codsequencia = 1000633 where codarq = 4006 and codcam = 22232");

    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4402,'jornadahoras_jornada_tiporegistro_in',4006,'0']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4007, 'gradeshorarios', 'Tabela com as grades de horários', 'rh190', '2016-12-13', 'Grades de horários', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22237,'rh190_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22238,'rh190_descricao','varchar(50)','Descrição da grade de horários','', 'Descrição',50,'f','t','f',0,'text','Descrição'],
      [22239,'rh190_database','date','Data base da grade de horários','null', 'Data Base',10,'f','f','f',1,'text','Data Base']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000634, 'gradeshorarios_rh190_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4008, 'gradeshorariosjornada', 'Tabela que vincula a grade de horário com as jornadas', 'rh191', '2016-12-13', 'Vinculo da grade de horário com as jornadas', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22240,'rh191_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22241,'rh191_gradehorarios','int4','Vínculo com a grade de horários','0', 'Grades de Horários',19,'f','f','f',1,'text','Grades de Horários'],
      [22242,'rh191_ordemhorario','int4','Ordenação dos horários','0', 'Ordem',19,'f','f','f',1,'text','Ordem'],
      [22243,'rh191_jornada','int4','Vínculo com a tabela de jornada','0', 'Jornada',19,'f','f','f',1,'text','Jornada']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000635, 'gradeshorariosjornada_rh191_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    $this->execute("update db_sysarqcamp set codsequencia = 1000635 where codarq = 4008 and codcam = 22240");

    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4401,'gradeshorariosjornada_gradeshorarios_ordemhorario_jornada_in',4008,'0']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4009, 'escalaservidor', 'Tabela com as escalas do servidor', 'rh192', '2016-12-13', 'Escalas do servidor', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22244,'rh192_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22245,'rh192_gradeshorarios','int4','Vínculo com a tabela de grades de horários','0', 'Grades de Horários',19,'f','f','f',1,'text','Grades de Horários'],
      [22246,'rh192_regist','int4','Matrícula do servidor','0', 'Matrícula',19,'f','f','f',1,'text','Matrícula'],
      [22247,'rh192_instit','int4','Código da instituição','0', 'Instituição',19,'f','f','f',1,'text','Instituição'],
      [22248,'rh192_dataescala','date','Data da escala','null', 'Data da Escala',10,'f','f','f',1,'text','Data da Escala']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000636, 'escalaservidor_rh192_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    $this->execute("update db_sysarqcamp set codsequencia = 1000636 where codarq = 4009 and codcam = 22244");

    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4400,'escalaservidor_gradeshorarios_regist_instit_in',4009,'0']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4010, 'assentamentofuncional', 'Tabela que vincula um assentamento afim de torná-lo um assentamento de vida funcional', 'rh193', '2016-12-13', 'Assentamento de vida funcional', 0, 'f', 't', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22249,'rh193_assentamento_funcional','int4','Vínculo com a tabela de assentamentos','0', 'Assentamento de Vida Funcional',19,'f','f','f',1,'text','Assentamento de Vida Funcional'],
      [22250,'rh193_assentamento_efetividade','int4','Vínculo com a tabela assenta','0', 'Assentamento de Efetividade',19,'f','f','f',1,'text','Assentamento de Efetividade']
    ]);
    ...
    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4403,'assentamentofuncional_un_in',4010,'1']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4011, 'pontoeletronicojustificativa', 'Tabela de justificativas do ponto eletrônico', 'rh194', '2016-12-21', 'Tabela de justificativas', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22294,'rh194_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22295,'rh194_descricao','varchar(50)','Descrição da justificativa','', 'Descrição',50,'f','t','f',0,'text','Descrição'],
      [22296,'rh194_sigla','varchar(3)','Sigla para a justificativa, será exibida no espelho ponto','', 'Sigla',3,'f','t','f',0,'text','Sigla'],
      [22297,'rh194_instituicao','int4','Vínculo com a instituição','0', 'Instituição',19,'f','f','f',1,'text','Instituição']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000642, 'pontoeletronicojustificativa_rh194_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    ...
    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4405,'pontoeletronicojustificativa_un_in',4011,'1']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4028, 'pontoeletronicojustificativatipoasse', 'Tabela de vínculo entre justificativas e tipos de assentamentos.', 'rh205', '2017-02-13', 'Vínculo entre justificativas e tipos assentamentos', 0, 'f', 't', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22357,'rh205_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22358,'rh205_pontoeletronicojustificativa','int4','Vínculo com a tabela de justificativas','0', 'Justificativa',19,'f','f','f',1,'text','Justificativa'],
      [22359,'rh205_tipoasse','int4','Vínculo com a tabela de tipos de assentamento','0', 'Tipo de Assentamento',19,'f','f','f',1,'text','Tipo de Assentamento']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000653, 'pontoeletronicojustificativatipoasse_rh205_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4012, 'pontoeletronicoconfiguracoeslotacao', 'Tabela de configurações de horas e rubricas para lançamento dos valores no ponto.', 'rh195', '2016-12-21', '', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22281,'rh195_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22283,'rh195_tolerancia','int4','Tolerância das batidas','0', 'Tolerância',19,'f','f','f',1,'text','Tolerância'],
      [22284,'rh195_hora_extra_50','text','Tempo quando começa a contar como hora extra de 50%','', 'Horas Extras 50% Até',5,'f','t','f',0,'text','Horas Extras 50%'],
      [22285,'rh195_hora_extra_75','text','Tempo para começar a contar como horas extras 75%','', 'Horas Extras 75%',5,'t','t','f',0,'text','Horas Extras 75%'],
      [22286,'rh195_hora_extra_100','text','Tempo que começa a contar como hora extra 100%','', 'Horas Extras 100%',5,'t','t','f',0,'text','Horas Extras 100%'],
      [22337,'rh195_lotacao','int4','Lotação','0', 'Lotação',19,'f','f','f',1,'text','Lotação'],
      [22338,'rh195_supervisor','int4','Supervisor responsável por assinar o espelho ponto','0', 'Supervisor',19,'f','f','f',1,'text','Supervisor']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000641, 'pontoeletronicoconfiguracoes_rh195_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4024, 'pontoeletronicoconfiguracoesgerais', 'Configurações gerais da efetividade', 'rh200', '2017-01-18', 'Configurações gerais da efetividade', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22327,'rh200_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22328,'rh200_instituicao','int4','Instituição da configuração','0', 'Instituição',19,'f','f','f',1,'text','Instituição'],
      [22329,'rh200_tipoasse_extra50diurna','int4','Horas extras 50% diurnas','0', 'Horas Extras 50% Diurna',19,'t','f','f',1,'text','Horas Extras 50% Diurna'],
      [22330,'rh200_tipoasse_extra75diurna','int4','Horas extras 75% diurnas','0', 'Horas Extras 75% Diurna',19,'t','f','f',1,'text','Horas Extras 75% Diurna'],
      [22331,'rh200_tipoasse_extra100diurna','int4','Horas extras 100% diurnas','0', 'Horas Extras 100% Diurna',19,'t','f','f',1,'text','Horas Extras 100% Diurna'],
      [22332,'rh200_tipoasse_extra50noturna','int4','Horas extras 50% noturnas','0', 'Horas Extras 50% Noturna',19,'t','f','f',1,'text','Horas Extras 50% Noturna'],
      [22333,'rh200_tipoasse_extra75noturna','int4','Horas extras 75% noturna','0', 'Horas Extras 75% Noturna',19,'t','f','f',1,'text','Horas Extras 75% Noturna'],
      [22334,'rh200_tipoasse_extra100noturna','int4','Horas extras 100% noturnas','0', 'Horas Extras 100% Noturna',19,'t','f','f',1,'text','Horas Extras 100% Noturna'],
      [22335,'rh200_tipoasse_adicionalnoturno','int4','Horas adicional noturno','0', 'Horas Adicional Noturno',19,'t','f','f',1,'text','Horas Adicional Noturno'],
      [22336,'rh200_tipoasse_falta','int4','Horas falta','0', 'Horas Falta',19,'t','f','f',1,'text','Horas Falta']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000649, 'pontoeletronicoconfiguracoesgerais_rh200_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    ...
    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4416,'pontoeletronicoconfiguracoesgerais_un_in',4024,'1']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4013, 'pontoeletronicoarquivo', 'Tabela que guarda o arquivo do ponto importado', 'rh196', '2016-12-21', 'Tabela com arquivo do ponto importado', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22274,'rh196_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22275,'rh196_instituicao','int4','Vínculo com a efetividade, instituição','0', 'Instituição',19,'f','f','f',1,'text','Instituição'],
      [22276,'rh196_efetividade_exercicio','int4','Vínculo com a efetividade, exercício','0', 'Exercício da Efetividade',19,'f','f','f',1,'text','Exercício da Efetividade'],
      [22277,'rh196_efetividade_competencia','varchar(2)','Vínculo com a efetividade, competência','', 'Competência da Efetividade',2,'f','t','f',0,'text','Competência da Efetividade'],
      [22278,'rh196_ano','int4','Ano da competência','0', 'Ano da Competência',19,'t','f','f',1,'text','Ano da Competência'],
      [22279,'rh196_mes','int4','Mês da competência','0', 'Mês da competência',19,'t','f','f',1,'text','Mês da competência'],
      [22280,'rh196_arquivo','oid','Arquivo de marcaçõs do ponto','', 'Arquivo',1,'f','f','f',0,'text','Arquivo']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000640, 'pontoeletronicoarquivo_rh196_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4014, 'pontoeletronicoarquivodata', 'Tabela com as datas de marcações do registro de ponto.', 'rh197', '2016-12-22', 'Datas das marcações do arquivo', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22263,'rh197_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22264,'rh197_pontoeletronicoarquivo','int4','Vínculo com o arquivo de marcações do ponto','0', 'Arquivo',19,'f','f','f',1,'text','Arquivo'],
      [22265,'rh197_data','date','Data do registro do ponto','null', 'Data',10,'f','f','f',1,'text','Data'],
      [22266,'rh197_matricula','int4','Mátricula do servidor','0', 'Mátricula',19,'t','f','f',1,'text','Mátricula'],
      [22267,'rh197_horas_trabalhadas','text','Horas trabalhadas','0', 'Horas Trabalhadas',5,'t','t','f',0,'text','Horas Trabalhadas'],
      [22269,'rh197_horas_falta','text','Horas de falta','0', 'Horas Falta',5,'t','t','f',0,'text','Horas Falta'],
      [22270,'rh197_horas_extras_50','text','Horas extras 50%','', 'Horas Extras 50%',5,'t','t','f',0,'text','Horas Extras 50%'],
      [22271,'rh197_horas_extras_75','text','Horas extras 75%','', 'Horas Extras 75%',5,'t','t','f',0,'text','Horas Extras 75%'],
      [22272,'rh197_horas_extras_100','text','Horas extras 100%','', 'Horas Extras 100%',5,'t','t','f',0,'text','Horas Extras 100%'],
      [22273,'rh197_horas_adicinal_noturno','text','Horas de adicional noturno','', 'Horas Adicional Noturno',5,'t','t','f',0,'text','Horas Adicional Noturno'],
      [22326,'rh197_pis','varchar(11)','Número PIS do servidor.','', 'PIS',11,'f','t','f',0,'text','PIS']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000639, 'pontoeletronicoarquivodata_rh197_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    ...
    $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
      [4404,'pontoeletronicoarquivodata_pontoeletronicoarquivo_data_pis_un_in',4014,'1']
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4015, 'pontoeletronicoarquivodataregistro', 'Tabela que guarda as marcações do arquivo de ponto eletrônico.', 'rh198', '2016-12-22', 'Marcações do arquivo de ponto', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22258,'rh198_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22259,'rh198_pontoeletronicoarquivodata','int4','Vínculo com a tabela que guarda das datas dos registros.','0', 'Data Registro',19,'f','f','f',1,'text','Data Registro'],
      [22260,'rh198_registro','text','Registros de entrada e saída do arquivo ponto','', 'Registro',5,'t','t','f',0,'text','Registro'],
      [22261,'rh198_registro_manual','bool','Informa se o registro foi feito manualmente','f', 'Registro Manual',1,'f','f','f',5,'text','Registro Manual'],
      [22262,'rh198_ordem','int4','Ordem dos registros do ponto no dia','0', 'Ordem',19,'f','f','f',1,'text','Ordem'],
      [22356,'rh198_data','date','Data da batida do ponto.','null', 'Data',10,'f','f','f',1,'text','Data']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000638, 'pontoeletronicoarquivodataregistro_rh198_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
    ...
    $this->tabela_db_sysprikey->insert(['codarq','codcam','sequen','camiden'], [
     ...
     $this->tabela_db_sysindices->insert(['codind', 'nomeind', 'codarq', 'campounico'], [
       [4417,'pontoeletronicoarquivodataregistro_pontoeletronicoarquivodata_in',4015,'0']
     ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4016, 'pontoeletronicoregistrojustificativa', 'Tabela que vincula uma justificativa à uma marcação de horário.', 'rh199', '2016-12-22', 'Vinculo entre justificativa e marcação', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22254,'rh199_sequencial','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22255,'rh199_pontoeletronicoarquivodataregistro','int4','Vínculo com o registro da marcação do ponto','0', 'Registro',19,'f','f','f',1,'text','Registro'],
      [22256,'rh199_pontoeletronicojustificativa','int4','Vínculo com a justificativa para marcação','0', 'Justificativa',19,'f','f','f',1,'text','Justificativa'],
      [22257,'rh199_tipo','char(1)','Tipo de justificativa P para parcial e T para total.','', 'Tipo de Justificativa',1,'f','t','f',0,'text','Tipo de Justificativa']
    ]);
    ...
    $this->tabela_db_syssequencia->insert(['codsequencia','nomesequencia','incrseq','minvalueseq','maxvalueseq','startseq','cacheseq'], [
      [1000637, 'pontoeletronicoregistrojustificativa_rh199_sequencial_seq', 1, 1, 9223372036854775807, 1, 1]
    ]);
  ...
  {
    $this->tabela_db_sysarquivo->insert(['codarq','nomearq','descricao','sigla','dataincl','rotulo','tipotabela','naolibclass','naolibfunc','naolibprog','naolibform'], [
      [4029, 'assentamentojustificativaperiodo', 'Tabela que determina se um assentamento de justificativa é de período específico ou integral', 'rh206', '2017-02-13', 'Período do assentamento de justificativa', 0, 'f', 'f', 't', 't' ]
    ]);
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'], [
      [22360,'rh206_codigo','int4','Código sequencial da tabela','0', 'Código',19,'f','f','f',1,'text','Código'],
      [22361,'rh206_periodo','int4','Período do assentamento de justificativa.','0', 'Período',19,'f','f','f',1,'text','Período']
    ]);
  ...
  public function upDDLTabelasEfetividade()
  {
    ...
    $this->table('tipoassedb_depart', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>['rh184_sequencial'], 'constraint'=>'tipoassedb_depart_sequencial_pk'])
      ->addColumn('rh184_sequencial', 'integer')
      ->addColumn('rh184_db_depart',  'integer')
      ->addColumn('rh184_tipoasse',   'integer')
      ->addForeignKey('rh184_db_depart', 'configuracoes.db_depart',   'coddepto', ['constraint'=>'tipoassedb_depart_db_depart_fk'])
      ->addForeignKey('rh184_tipoasse',  'recursoshumanos.tipoasse',  'h12_codigo', ['constraint'=>'tipoassedb_depart_tipoasse_fk'])
      ->addIndex(['rh184_db_depart','rh184_tipoasse'],  ['unique'=>false, 'name'=>'tipoassedb_depart_db_depart_tipoasse_in'])
      ->save();
    ...
    $this->table('db_departrhlocaltrab', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>['rh185_sequencial'], 'constraint'=>'db_departrhlocaltrab_sequencial_pk'])
      ...
      ->addForeignKey('rh185_db_depart',   'configuracoes.db_depart',  'coddepto',                          ['constraint'=>'db_departrhlocaltrab_rh185_db_depart_fk'])
      ->addForeignKey(['rh185_rhlocaltrab', 'rh185_instit'], 'pessoal.rhlocaltrab', ['rh55_codigo', 'rh55_instit'], ['constraint'=>'db_departrhlocaltrab_rhlocaltrab_fk'])
      ->addIndex(['rh185_db_depart','rh185_rhlocaltrab','rh185_instit'],  ['unique'=>false, 'name'=>'db_departrhlocaltrab_db_depart_rhlocaltrab_instit_in'])
      ->save();
    ...
    $this->table('configuracoesdatasefetividade', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>['rh186_exercicio', 'rh186_competencia', 'rh186_instituicao'], 'constraint'=>'configuracoesdatasefetividade_pk'])
      ->addColumn('rh186_exercicio',                  'integer', ['null'=>true])
      ->addColumn('rh186_competencia',                'string',  ['null'=>true, 'limit'=>2])
      ->addColumn('rh186_datainicioefetividade',      'date',    ['null'=>true])
      ->addColumn('rh186_datafechamentoefetividade',  'date',    ['null'=>true])
      ->addColumn('rh186_dataentregaefetividade',     'date',    ['null'=>true])
      ->addColumn('rh186_processado',                 'boolean', ['null'=>true, 'default'=>false])
    ...
    $this->table('tiporegistro', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh187_sequencial', 'constraint'=>'tiporegistro_sequencial_pk'])
      ->addColumn('rh187_sequencial',  'integer')
      ->addColumn('rh187_descricao',   'string',  ['limit'=>50])
      ->save();
    ...
    $this->table('jornada', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh188_sequencial', 'constraint'=>'jornada_sequencial_pk'])
      ->addColumn('rh188_sequencial',   'integer')
      ->addColumn('rh188_descricao',    'string',    ['limit'=>50])
      ->addColumn('rh188_fixo',         'boolean')
      ->addColumn('rh188_tipo',         'char',      ['null'=>true, 'limit' => 1])
      ->save();
    ...
    $this->table('jornadahoras', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh189_sequencial', 'constraint'=>'jornadahoras_sequencial_pk'])
      ->addColumn('rh189_sequencial',    'integer')
      ->addColumn('rh189_jornada',       'integer')
      ->addColumn('rh189_tiporegistro',  'integer')
      ->addColumn('rh189_hora',          'string', ['limit'=>5])
      ->addForeignKey('rh189_jornada',      'recursoshumanos.jornada',      'rh188_sequencial',  ['constraint'=>'jornadahoras_rh189_jornada_fk'])
      ->addForeignKey('rh189_tiporegistro', 'recursoshumanos.tiporegistro', 'rh187_sequencial',  ['constraint'=>'jornadahoras_rh189_tiporegistro_fk'])
      ->addIndex(['rh189_jornada', 'rh189_jornada'],  ['unique'=>false, 'name'=>'jornadahoras_jornada_tiporegistro_in'])
      ->save();
    ...
    $this->table('gradeshorarios', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh190_sequencial', 'constraint'=>'gradeshorarios_sequencial_pk'])
      ->addColumn('rh190_sequencial', 'integer')
      ->addColumn('rh190_descricao',  'string',  ['limit'=>50])
    ...
    $this->table('gradeshorariosjornada', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh191_sequencial', 'constraint'=>'gradeshorariosjornada_sequencial_pk'])
      ...
      ->addForeignKey('rh191_gradehorarios',   'recursoshumanos.gradeshorarios',  'rh190_sequencial',  ['constraint'=>'gradeshorariosjornada_rh191_gradehorarios_fk'])
      ->addForeignKey('rh191_jornada',          'recursoshumanos.jornada',         'rh188_sequencial',  ['constraint'=>'gradeshorariosjornada_rh191_jornada_fk'])
      ->addIndex(['rh191_gradehorarios', 'rh191_ordemhorario', 'rh191_jornada'],  ['unique'=>false, 'name'=>'gradeshorariosjornada_gradeshorarios_ordemhorario_jornada_in'])
      ->save();
    ...
    $this->table('escalaservidor', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh192_sequencial', 'constraint'=>'escalaservidor_sequencial_pk'])
      ...
      ->addForeignKey('rh192_gradeshorarios',  'recursoshumanos.gradeshorarios',  'rh190_sequencial',   ['constraint'=>'escalaservidor_rh192_gradeshorarios_fk'])
      ->addForeignKey('rh192_instit',          'configuracoes.db_config',         'codigo',             ['constraint'=>'escalaservidor_rh192_instit_fk'])
      ->addIndex(['rh192_gradeshorarios', 'rh192_regist', 'rh192_instit'],  ['unique'=>false, 'name'=>'escalaservidor_gradeshorarios_regist_instit_in'])
      ->save();
    ...
    $this->table('assentamentofuncional', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh193_assentamento_funcional', 'constraint'=>'assentamentofuncional_assentamento_funcional_pk'])
      ->addColumn('rh193_assentamento_funcional',     'integer')
      ->addColumn('rh193_assentamento_efetividade',   'integer', ['null'=>true])
      ->addForeignKey('rh193_assentamento_funcional',    'recursoshumanos.assenta',   'h16_codigo',   ['constraint'=>'assentamentofuncional_rh193_assentamento_funcional_fk'])
      ->addForeignKey('rh193_assentamento_efetividade',  'recursoshumanos.assenta',   'h16_codigo',   ['constraint'=>'assentamentofuncional_rh193_assentamento_efetividade_fk'])
      ->addIndex('rh193_assentamento_funcional',  ['unique'=>false, 'name'=>'assentamentofuncional_un_in'])
      ->save();
  }
  ...
  public function upDDLTabelasPontoEletronico()
  {
    ...
    $this->table('pontoeletronicojustificativa', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh194_sequencial', 'constraint'=>'pontoeletronicojustificativa_sequencial_pk'])
         ->addColumn('rh194_sequencial',      'integer')
         ->addColumn('rh194_descricao',       'string',     ['limit'=>50])
         ->addColumn('rh194_sigla',           'string',     ['limit'=>3])
         ->addColumn('rh194_instituicao',     'integer')
         ->addForeignKey('rh194_instituicao',    'configuracoes.db_config',     'codigo',   ['constraint'=>'pontoeletronicojustificativa_rh194_instituicao_fk'])
         ->addIndex(['rh194_sigla', 'rh194_instituicao'], ['unique'=>true, 'name'=>'pontoeletronicojustificativa_un_in'])
         ->save();
    ...
    $this->table('pontoeletronicoconfiguracoeslotacao', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh195_sequencial', 'constraint'=>'pontoeletronicoconfiguracoeslotacao_sequencial_pk'])
         ->addColumn('rh195_sequencial',                   'integer')
         ->addColumn('rh195_tolerancia',                   'integer')
         ->addColumn('rh195_hora_extra_50',                'string')
         ->addColumn('rh195_hora_extra_75',                'string',     ['null'=>true])
         ->addColumn('rh195_hora_extra_100',               'string',     ['null'=>true])         
         ->addColumn('rh195_lotacao',                      'integer')
         ->addColumn('rh195_supervisor',                   'integer')
         ->addForeignKey('rh195_lotacao',        'pessoal.rhlota',              'r70_codigo',   ['constraint'=>'pontoeletronicoconfiguracoeslotacao_rh195_lotacao_fk'])
         ->addForeignKey('rh195_supervisor',     'pessoal.rhpessoal',           'rh01_regist',  ['constraint'=>'pontoeletronicoconfiguracoeslotacao_rh195_supervisor_fk'])
         ->save();
    ...
    $this->table('pontoeletronicoconfiguracoesgerais', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh200_sequencial', 'constraint'=>'pontoeletronicoconfiguracoesgerais_sequencial_pk'])
         ->addColumn('rh200_sequencial',                   'integer')
         ->addColumn('rh200_instituicao',                  'integer')
         ->addColumn('rh200_tipoasse_extra50diurna',       'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_extra75diurna',       'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_extra100diurna',      'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_extra50noturna',      'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_extra75noturna',      'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_extra100noturna',     'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_adicionalnoturno',    'integer',    ['null'=>true])
         ->addColumn('rh200_tipoasse_falta',               'integer',    ['null'=>true])
         ->addForeignKey('rh200_instituicao',    'configuracoes.db_config',     'codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_instituicao_fk'])
         ->addForeignKey('rh200_tipoasse_extra50diurna',     'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_extra50diurna_fk'])
         ->addForeignKey('rh200_tipoasse_extra75diurna',     'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_extra75diurna_fk'])
         ->addForeignKey('rh200_tipoasse_extra100diurna',    'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_extra100diurna_fk'])
         ->addForeignKey('rh200_tipoasse_extra50noturna',    'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_extra50noturna_fk'])
         ->addForeignKey('rh200_tipoasse_extra75noturna',    'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_extra75noturna_fk'])
         ->addForeignKey('rh200_tipoasse_extra100noturna',   'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_extra100noturna_fk'])
         ->addForeignKey('rh200_tipoasse_adicionalnoturno',  'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_adicionalnoturno_fk'])
         ->addForeignKey('rh200_tipoasse_falta',             'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_falta_fk'])
         ->addIndex('rh200_instituicao',    ['unique'=>true, 'name'=>'pontoeletronicoconfiguracoesgerais_un_in'])
         ->save();
    ...
    $this->table('pontoeletronicoarquivo', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh196_sequencial', 'constraint'=>'pontoeletronicoarquivo_sequencial_pk'])
         ->addColumn('rh196_sequencial',               'integer')
         ->addColumn('rh196_efetividade_exercicio',    'integer')
         ->addColumn('rh196_efetividade_competencia',  'string',   ['limit'=>2])
         ->addColumn('rh196_instituicao',              'integer')
         ->addColumn('rh196_ano',                      'integer',  ['null'=>true])
         ->addColumn('rh196_mes',                      'integer',  ['null'=>true])
         ->addForeignKey('rh196_instituicao',                 'configuracoes.db_config',        'codigo',                                        ['constraint'=>'pontoeletronicoarquivo_rh196_instituicao_fk'])
         ->addForeignKey(['rh196_efetividade_exercicio', 'rh196_efetividade_competencia', 'rh196_instituicao'], 'recursoshumanos.configuracoesdatasefetividade', ['rh186_exercicio', 'rh186_competencia', 'rh186_instituicao'], ['constraint'=>'pontoeletronicoarquivo_rh196_efetividade_fk'])
         ->save();
    ...
    $this->table('pontoeletronicoarquivodata', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh197_sequencial', 'constraint'=>'pontoeletronicoarquivodata_sequencial_pk'])
         ->addColumn('rh197_sequencial',                'integer')
         ->addColumn('rh197_pontoeletronicoarquivo',    'integer')
         ->addColumn('rh197_data',                      'date')
         ->addColumn('rh197_matricula',                 'integer', ['null'=>true])
         ->addColumn('rh197_horas_trabalhadas',         'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_atraso',              'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_falta',               'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_extras_50',           'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_extras_75',           'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_extras_100',          'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_adicinal_noturno',    'string',  ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_pis',                       'string',  ['limit'=>11])
         ->addForeignKey('rh197_pontoeletronicoarquivo',  'recursoshumanos.pontoeletronicoarquivo',     'rh196_sequencial',   ['constraint'=>'pontoeletronicoarquivodata_rh197_pontoeletronicoarquivo_fk'])
         ->addIndex(['rh197_pontoeletronicoarquivo', 'rh197_data', 'rh197_pis'],    ['unique'=>true, 'name'=>'pontoeletronicoarquivodata_pontoeletronicoarquivo_data_pis_un_in'])
      ->save();
    ...
    $this->table('pontoeletronicoarquivodataregistro', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh198_sequencial', 'constraint'=>'pontoeletronicoarquivodataregistro_sequencial_pk'])
         ->addColumn('rh198_sequencial',                   'integer')
         ->addColumn('rh198_pontoeletronicoarquivodata',   'integer')
         ->addColumn('rh198_registro',                     'string', ['null' => true])
         ->addColumn('rh198_registro_manual',              'boolean')
         ->addColumn('rh198_ordem',                        'integer')
         ->addColumn('rh198_data',                         'date')
         ->addForeignKey('rh198_pontoeletronicoarquivodata',    'recursoshumanos.pontoeletronicoarquivodata',     'rh197_sequencial',   ['constraint'=>'pontoeletronicoarquivodataregistro_rh198_pontoeletronicoarquivodata_fk'])
         ->save();
    ...
    $this->table('pontoeletronicoregistrojustificativa', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh199_sequencial', 'constraint'=>'pontoeletronicoregistrojustificativa_sequencial_pk'])
         ...
         ->addColumn('rh199_tipo',                                 'char',      ['limit'=>1])
         ->addForeignKey('rh199_pontoeletronicoarquivodataregistro', 'recursoshumanos.pontoeletronicoarquivodataregistro',  'rh198_sequencial',   ['constraint'=>'pontoeletronicoarquivodataregistro_fk'])
         ->addForeignKey('rh199_pontoeletronicojustificativa',       'recursoshumanos.pontoeletronicojustificativa',        'rh194_sequencial',   ['constraint'=>'pontoeletronicojustificativa_fk'])
         ->save();
    ...
    $this->table('pontoeletronicojustificativatipoasse', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>'rh205_sequencial', 'constraint'=>'pontoeletronicojustificativatipoasse_sequencial_pk'])
         ->addColumn('rh205_sequencial',                         'integer')
         ->addColumn('rh205_pontoeletronicojustificativa',       'integer')
         ->addColumn('rh205_tipoasse',                           'integer')
         ->addForeignKey('rh205_pontoeletronicojustificativa', 'recursoshumanos.pontoeletronicojustificativa',  'rh194_sequencial',   ['constraint'=>'pontoeletronicojustificativa_fk'])
         ->addForeignKey('rh205_tipoasse',                     'recursoshumanos.tipoasse',                      'h12_codigo',         ['constraint'=>'tipoasse_fk'])
         ->save();
    ...
    $this->table('assentamentojustificativaperiodo', ['schema'=>'recursoshumanos', 'id'=>false, 'primary_key'=>['rh206_codigo', 'rh206_periodo'], 'constraint'=>'assentamentojustificativa_codigo_pk'])
         ->addColumn('rh206_codigo',                 'integer')
         ->addColumn('rh206_periodo',                'integer')
         ->addForeignKey('rh206_codigo',       'recursoshumanos.assenta',       'h16_codigo',         ['constraint'=>'assentamento_fk'])
         ->save();
  }

  public function downDDL()
  {
    $sTableJustificativaTipoasse = $this->table('pontoeletronicojustificativatipoasse', ['schema'=>'recursoshumanos']);
    ...
    $sTableConfiguracoesGerais = $this->table('pontoeletronicoconfiguracoesgerais', ['schema'=>'recursoshumanos']);
    ...
    $this->table('pontoeletronicojustificativa', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE IF EXISTS recursoshumanos.pontoeletronicojustificativa_rh194_sequencial_seq");

    $sTableAssentamentoJustificativaPeriodo = $this->table('assentamentojustificativaperiodo', ['schema'=>'recursoshumanos']);

    if($sTableAssentamentoJustificativaPeriodo->exists()) {
      $this->table('assentamentojustificativaperiodo', ['schema'=>'recursoshumanos'])->drop();
    }

    $this->table('assentamentofuncional', ['schema'=>'recursoshumanos'])->drop();

    $this->table('escalaservidor', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.escalaservidor_rh192_sequencial_seq");

    $this->table('gradeshorariosjornada', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.gradeshorariosjornada_rh191_sequencial_seq");

    $this->table('gradeshorarios', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.gradeshorarios_rh190_sequencial_seq");

    $this->table('jornadahoras', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.jornadahoras_rh189_sequencial_seq");

    $this->table('jornada', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.jornada_rh188_sequencial_seq");

    $this->table('tiporegistro', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.tiporegistro_rh187_sequencial_seq");

    $this->table('configuracoesdatasefetividade', ['schema'=>'recursoshumanos'])->drop();

    $this->table('db_departrhlocaltrab', ['schema'=>'recursoshumanos'])->drop();
    $this->execute("DROP SEQUENCE recursoshumanos.db_departrhlocaltrab_rh185_sequencial_seq");

    $this->table('tipoassedb_depart', ['schema'=>'recursoshumanos'])->drop();
  ...
  public function adicionarMenus()
  {
    ...
    $this->table('db_itensmenu', ['schema'=>'configuracoes'])->insert(['id_item', 'descricao', 'help', 'funcao', 'itemativo', 'manutencao', 'desctec', 'libcliente'], [
      [ 10360 ,'Mês Atual' ,'Mês Atual' ,'rec2_novagradeefetividade001.php' ,'1' ,'1' ,'Grade de Efetividade (Novo)' ,'true' ],
      [ 10361 ,'Próximo Mês' ,'Próximo Mês' ,'rec2_consultagradeefetividade001.php' ,'1' ,'1' ,'Consulta Outras Grades de Efetividade' ,'true' ],
      [ 10365 ,'Efetividade' ,'Efetividade' ,'' ,'1' ,'1' ,'Efetividade' ,'true' ],
      [ 10366 ,'Jornadas' ,'Jornadas' ,'rec4_cadastrojornadas001.php' ,'1' ,'1' ,'Cadastro de Jornadas.' ,'true' ],
      [ 10367 ,'Escala de Trabalho' ,'Escala de Trabalho' ,'rec4_cadastrogradeshorarios001.php' ,'1' ,'1' ,'Cadastro de Escala de Trabalho.' ,'true' ],
      [ 10368 ,'Efetividade' ,'Efetividade' ,'' ,'1' ,'1' ,'Procedimentos da Efetividade.' ,'true' ],
      [ 10369 ,'Parâmetros' ,'Parâmetros' ,'' ,'1' ,'1' ,'Parâmetros da efetividade.' ,'true' ],
      [ 10370 ,'Vínculo Assentamentos/Afastamentos por Departamento' ,'Vínculo Assentamentos/Afastamentos por Departamento' ,'rec4_tipoassedb_depart001.php' ,'1' ,'1' ,'Vínculo Assentamentos/Afastamentos por Departamento' ,'true' ],
      [ 10371 ,'Vínculo Locais de Trabalho por Departamento' ,'Vínculo Locais de Trabalho por Departamento' ,'rec4_db_departrhlocaltrab001.php' ,'1' ,'1' ,'Vínculo Locais de Trabalho por Departamento' ,'true' ],
      [ 10372 ,'Períodos de Efetividade' ,'Períodos de Efetividade' ,'rec4_configuracoesefetividade001.php' ,'1' ,'1' ,'Períodos de Efetividade' ,'true' ],
      [ 10373 ,'Encerrar Período' ,'Encerrar Período' ,'rec4_encerramentoefetividade001.php' ,'1' ,'1' ,'Encerrar Período' ,'true' ],
      [ 10374 ,'Reabrir Período' ,'Reabrir Período' ,'rec4_reabertuarefetividade001.php' ,'1' ,'1' ,'Reabrir Período' ,'true' ],
      [ 10375 ,'Manutenção da Escala de Funcionários' ,'Manutenção da Escala de Funcionários' ,'rec4_escalaservidores001.php' ,'1' ,'1' ,'Manutenção da Escala de Funcionários' ,'true' ],
      [ 10376 ,'Assentamentos de Efetividade' ,'Assentamentos de Efetividade' ,'' ,'1' ,'1' ,'Assentamentos de Efetividade' ,'true' ],
      [ 10377 ,'Inclusão' ,'Inclusão' ,'rec1_assenta_efetividade001.php' ,'1' ,'1' ,'Inclusão de assentamentos de efetividade.' ,'true' ],
      [ 10378 ,'Alteração' ,'Alteração' ,'rec1_assenta_efetividade002.php' ,'1' ,'1' ,'Alteração de assentamentos de efetividade.' ,'true' ],
      [ 10379 ,'Exclusão' ,'Exclusão' ,'rec1_assenta_efetividade003.php' ,'1' ,'1' ,'Exclusão de assentamentos de efetividade.' ,'true' ],
      [ 10380 ,'Importar Assentamentos para Vida Funcional' ,'Importar Assentamentos para Vida Funcional' ,'rec4_importarassentamentos001.php' ,'1' ,'1' ,'Importar Assentamentos para Vida Funcional' ,'true' ],
      [ 10384 ,'Ponto Eletrônico' ,'Ponto Eletrônico' ,'' ,'1' ,'1' ,'Menus referentes a procedimentos do ponto eletrônico.' ,'true' ],
      [ 10385 ,'Importar Arquivo' ,'Importar Arquivo' ,'rec4_pontoeletronicoimportararquivo.php' ,'1' ,'1' ,'Importação do arquivo do ponto.' ,'true' ],
      [ 10388 ,'Ponto Eletrônico' ,'Ponto Eletrônico' ,'' ,'1' ,'1' ,'Relatórios do ponto eletrônico' ,'true' ],
      [ 10389 ,'Espelho Ponto' ,'Espelho Ponto' ,'rec2_espelhoponto001.php' ,'1' ,'1' ,'Espelho ponto dos servidores.' ,'true' ],
      [ 10391 ,'Configurações' ,'Configurações' ,'rec4_pontoeletronicoconfiguracoes001.php' ,'1' ,'1' ,'Configurações de assentamentos/rubricas do ponto eletrônico.' ,'true' ],
      [ 10392 ,'Manutenção' ,'Manutenção' ,'rec4_pontoeletronicomanutencao001.php' ,'1' ,'1' ,'Rotina para manutenção do ponto eletrônico dos servidores.' ,'true' ],
      [ 10399 ,'Efetividade' ,'Efetividade' ,'' ,'1' ,'1' ,'Menu com os relatórios referentes a efetividade.' ,'true' ],
    ])->saveData();

    //vínculos dos menus
    $this->table('db_menu', ['schema'=>'configuracoes'])->insert(['id_item', 'id_item_filho', 'menusequencia', 'modulo'], [
      [ 29    ,10365 ,272 ,2323 ],
      [ 10365 ,10366 ,1   ,2323 ],
      [ 10365 ,10367 ,2   ,2323 ],
      [ 30    ,10388 ,464 ,2323 ],
  ...
  public function migrarDados()
  {
    ...
    if(empty($tiposRegistro)) {
      ...
      $this->table('tiporegistro', ['schema'=>'recursoshumanos'])->insert(['rh187_sequencial', 'rh187_descricao'], [[$idTiporegistro[0], 'ENTRADA 1']])->saveData();

      $idTiporegistro = $this->fetchRow("SELECT nextval('recursoshumanos.tiporegistro_rh187_sequencial_seq')");
      $this->table('tiporegistro', ['schema'=>'recursoshumanos'])->insert(['rh187_sequencial', 'rh187_descricao'], [[$idTiporegistro[0], 'SAIDA 1']])->saveData();

      $idTiporegistro = $this->fetchRow("SELECT nextval('recursoshumanos.tiporegistro_rh187_sequencial_seq')");
      $this->table('tiporegistro', ['schema'=>'recursoshumanos'])->insert(['rh187_sequencial', 'rh187_descricao'], [[$idTiporegistro[0], 'ENTRADA 2']])->saveData();

      $idTiporegistro = $this->fetchRow("SELECT nextval('recursoshumanos.tiporegistro_rh187_sequencial_seq')");
      $this->table('tiporegistro', ['schema'=>'recursoshumanos'])->insert(['rh187_sequencial', 'rh187_descricao'], [[$idTiporegistro[0], 'SAIDA 2']])->saveData();
    }
    ...
    if(empty($jornadas)) {
      ...
      $this->table('jornada', ['schema'=>'recursoshumanos'])->insert(['rh188_sequencial', 'rh188_descricao', 'rh188_fixo', 'rh188_tipo'], [[$idJornada[0], 'DSR', 't', 'D']])->saveData();

      $idJornada = $this->fetchRow("SELECT nextval('recursoshumanos.jornada_rh188_sequencial_seq')");
      $this->table('jornada', ['schema'=>'recursoshumanos'])->insert(['rh188_sequencial', 'rh188_descricao', 'rh188_fixo', 'rh188_tipo'], [[$idJornada[0], 'FOLGA', 't', 'F']])->saveData();
    }
  ...
  public function adicionarNaturezaAssentamento()
  {
    ...
    if(empty($naturezasTipoAssentamentos)) {
      ...
      $this->table('naturezatipoassentamento', ['schema'=>'pessoal'])->insert(['rh159_sequencial', 'rh159_descricao'], [
        [4, 'Ponto Eletrônico'],
        [5, 'Justificativa'],
        [6, 'Dia Extra']
      ])->saveData();
  ...
  public function adicionarFeriadosPadrao()
  {
    ...
    foreach ($instituicoes as $instituicao) {
      ...
      if(empty($calendario)) {
        ...
        $this->table('rhcadcalend', ['schema'=>'pessoal'])->insert(['rh53_instit', 'rh53_descr'], [
          [$codigoInstituicao, 'FERIADOS PONTO ELETRÔNICO']
        ])->saveData();
        ...
        if(!empty($calendario)) {
          ...
          if(empty($datasFeriados)) {
            ...
            $this->table('calendf', ['schema'=>'pessoal'])->insert(['r62_calend', 'r62_data'], [
              [$calendario['rh53_calend'], '2017-01-01'], // CONFRATERNIZAÇÃO UNIVERSAL
              [$calendario['rh53_calend'], '2017-04-14'], // PAIXÃO DE CRISTO
              [$calendario['rh53_calend'], '2017-04-21'], // TIRADENTES
              [$calendario['rh53_calend'], '2017-05-01'], // DIA MUNDIAL DO TRABALHO
              [$calendario['rh53_calend'], '2017-09-07'], // INDEPENDÊNCIA DO BRASIL
              [$calendario['rh53_calend'], '2017-10-12'], // NOSSA SENHORA APARECIDA
              [$calendario['rh53_calend'], '2017-11-02'], // FINADOS
              [$calendario['rh53_calend'], '2017-11-15'], // PROCLAMAÇÃO DA REPÚBLICA
              [$calendario['rh53_calend'], '2017-12-25']  // NATAL
            ])->saveData();
          }
        }
      }
  ...
  public function incluirFormulaPadraoHorasPonto()
  {
    ...
    if(empty($formulasHorasPonto)) {
      $this->table('db_formulas', ['schema'=>'configuracoes'])->insert(['db148_nome','db148_descricao','db148_formula'], [
        ['PONTO_HORA','RETORNA AS HORAS EXTRAS OU FALTAS OU DE ADICIONAL NOTURNO','select h16_perc from assenta where h16_codigo = [CODIGO_ASSENTAMENTO]']
      ])->saveData();
    }
  }
}
