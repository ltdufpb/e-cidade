...
class M8184 extends PostgresMigration
{
  ...
  public function upPRETabela_pontoeletronicoconfiguracoesgerais()
  {
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'],[
        [22395,'rh200_tipoasse_faltas_dsr','int4','Assentamentos de faltas para desconto do DSR','0', 'Faltas desconto DSR',19,'f','f','f',1,'text','Faltas desconto DSR']
    ]);
  ...
  public function upPRETabela_pontoeletronicoarquivodata()
  {
    ...
    $this->tabela_db_syscampo->insert(['codcam','nomecam','conteudo','descricao','valorinicial','rotulo','tamanho','nulo','maiusculo','autocompl','aceitatipo','tipoobj','rotulorel'],[
      [22396,'rh197_horas_extras_50_n','text','Horas extras 50% noturna','0', 'Horas Extras 50% Noturna',5,'t','f','f',1,'text','Horas Extras 50% Noturna'],
      [22397,'rh197_horas_extras_75_n','text','Horas extras 75% noturnas','0', 'Horas Extras 75% Noturna',5,'t','f','f',1,'text','Horas Extras 75% Noturna'],
      [22398,'rh197_horas_extras_100_n','text','Horas extras 100% noturnas','0', 'Horas Extras 100% Noturna',5,'t','f','f',1,'text','Horas Extras 100% Noturna']
    ]);
  ...
  {
    $this->table('pontoeletronicoconfiguracoesgerais', ['schema'=>'recursoshumanos'])
         ->addColumn('rh200_tipoasse_faltas_dsr',       'integer',    ['null'=>true])
         ->addForeignKey('rh200_tipoasse_faltas_dsr',   'recursoshumanos.tipoasse',     'h12_codigo',       ['constraint'=>'pontoeletronicoconfiguracoesgerais_rh200_tipoasse_faltas_dsr_fk'])
         ->save();
    ...
    $this->table('pontoeletronicoarquivodata', ['schema'=>'recursoshumanos'])
         ->addColumn('rh197_horas_extras_50_n',       'string',    ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_extras_75_n',       'string',    ['limit'=>5, 'null'=>true])
         ->addColumn('rh197_horas_extras_100_n',      'string',    ['limit'=>5, 'null'=>true])
         ->save();
  }
  ...
  public function incluirFormulaPadraoHorasPonto()
  {
    ...
    if(empty($formulasHorasPonto)) {
      $this->table('db_formulas', ['schema'=>'configuracoes'])->insert(['db148_nome','db148_descricao','db148_formula'], [
        ['FALTAS_DSR','RETORNA A QUANTIDADE DE FALTAS PARA O DESCONTO DO DSR','select h16_quant from assenta where h16_codigo = [CODIGO_ASSENTAMENTO]']
      ])->saveData();
    }
  }
}
