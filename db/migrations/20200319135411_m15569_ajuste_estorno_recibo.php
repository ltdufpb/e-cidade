...
class M15569AjusteEstornoRecibo extends PostgresMigration
{
    public function up()
    {
        $this->execute(<<<SQL_WRAP
                    ...
                    IPTERM               alias for \$7;
                    INSTIT               alias for \$8;
                    iCodigoGrupo         alias for \$9; --Codigo do grupo de autentica\xe7\xe3o
                    ...
                    if NUMPAR = 0 then
                        ...
                        -- ESTORNO DE NUMPRE
                        -- O estorno de debitos com o numpre original do d\xe9bito em aberto so \xe9 realizado apos todos os numpares quitados.
                        -- 1 - Pode estar em um recibo normal pago, se estiver tem que ter arquivo banc\xe1rio de pagamento autenticado.
                        -- 2 - Pode estar em um recibo carne pago via baixa de banco. Se estiver tem que ter arquivo banc\xe1rio de pagamento autenticado.
                        -- 3 - Pode estar em um recibo carne pago, direto no caixa. Se for pode estornar. Esse processo de pagamento gera autentica\xe7\xe3o.
                        select sum(k00_valor),
                            case when
                                ...
                                -- Tem numpre em recibo pago e n\xe3o esta autenticado
                                -- Tem numpre pado por recibo carne e n\xe3o esta autenticado
                                (
                                select sum(1)
                                    from disbanco
                        ...
                        -- 1 - Tem que ter arquivo banc\xe1rio de pagamento autenticado.
                        select sum(k00_valor),
                                case when
                                (select sum(1)
                                    from disbanco
                        ...
                        -- Um estorno de d\xe9bito por numpre e numpar original, pode ser feito indiferente do meio de pagamento.
                        -- Um estorno de d\xe9bito por numpre e numpar original, pode ser feito indiferente do status dos outros numpar's do d\xe9bito.
                        -- 1 - Pode estar em um recibo normal pago, se estiver tem que ter arquivo banc\xe1rio de pagamento autenticado.
                        -- 2 - Pode estar em um recibo carne pago via baixa de banco. Se estiver tem que ter arquivo banc\xe1rio de pagamento autenticado.
                        -- 3 - Pode estar em um recibo carne pago, direto no caixa. Se for pode estornar. Esse processo de pagamento gera autentica\xe7\xe3o.
                        select sum(k00_valor),
                    ...
                    if iAutenticado is null then
                        ...
                        rtp_autenticacao.erro     := true;
                        rtp_autenticacao.mensagem := '6 D\xc9BITO A RECEBER ESTORNO N\xc3O FOI AUTENTICADO - ' || NUMPRE;
                    ...
                    IF NOT valor IS NULL THEN
                        ...
                        LOOP
                            ...
                            LOOP
                                ...
                                -- agora verifica a institui\xe7\xe3o da receita,
                                -- para impedir estorno de receita que n\xe3o seja da institui\xe7\xe3o corrente
                                SELECT K02_TIPO
                                INTO VTIPO
                                FROM TABREC
                                ...
                                rtp_autenticacao.erro     := true;
                                rtp_autenticacao.mensagem := '5 RECEITA ' || GRAVA_CORNUMP.K00_RECEIT || ' DE INSTITUI\xc7\xc3O DIFERENTE';
                                return rtp_autenticacao;
                        ...
                        -- Verifica se o estorno \xe9 referente a um recibo avulso de pagamento parcial
                        ...
                        -- Caso seja pagamento parcial ent\xe3o e percorrido todos os valores abatidos
                        ...
                        if iTipoAbatimento = 1 then
                            ...
                            if rAbatimentos.arrecad is null then
                                ...
                                -- Verifica o d\xe9bito foi quitado pelo pagamento parcial
                                select *
                                ...
                                raise exception 'OPERA\xc7\xc3O CANCELADA, EXISTEM D\xc9BITOS DE ORIGEM DO PAGAMENTO PARCIAL INFORMADO QUE N\xc3O EST\xc3O EM ABERTO!';
