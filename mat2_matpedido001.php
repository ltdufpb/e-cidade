...
if (isset($perini) || isset($perfim)){
 ...
 if ( trim((string) $perini) != "" && trim((string) $perfim) == "" ) {
   $where_matpedido .= " and m97_data >= '".implode('-',array_reverse(explode('/',(string) $perini)))."'";

 // verifica se somente o mês final foi preenchido
 } else if ( trim((string) $perfim) != "" && trim((string) $perini) == "" ) {
   $where_matpedido .= " and  m97_data <= '".implode('-',array_reverse(explode('/',(string) $perfim)))."'";

 // se todos períodos foram preenchidos
 } else if ( trim((string) $perini) != "" && trim((string) $perfim) != "" ) {
   $where_matpedido .= " and m97_data between '".implode('-',array_reverse(explode('/',(string) $perini)))."' and '".implode('-',array_reverse(explode('/',(string) $perfim)))."'";
 }
