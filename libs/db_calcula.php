...
class Crypt_HCEMD5 {
     ...
     */
    public $key, $rand = null) {
        if (!isset($rand)) {
            mt_srand((double)microtime() * 32767);
            $rand = random_int(1, 32767);
            $rand = pack('i*', $rand);
        }
        $this->rand = $rand;
	}
    ...
    function encrypt($data)
    {
        ...
        for ($i = 0; $i < $data_size; $i++) {
            $mod = $i % 16;
            if (($mod == 0) && ($i > 15)) {
       			$tmparr = [$ans[$i - 15],  $ans[$i - 14], $ans[$i - 13], $ans[$i - 12], $ans[$i - 11], $ans[$i - 10], $ans[$i - 9], $ans[$i - 8], $ans[$i - 7], $ans[$i - 6], $ans[$i - 5], $ans[$i - 4], $ans[$i - 3], $ans[$i - 2], $ans[$i - 1], $ans[$i] ];
    ...
    function decrypt($data)
    {
        ...
        for ($i = 0; $i < $data_size; $i++) {
            $mod = $i % 16;
            if (($mod == 0) && ($i > 15)) {
       			$tmparr = [$data[$i - 15], $data[$i - 14], $data[$i - 13], $data[$i - 12], $data[$i - 11], $data[$i - 10], $data[$i - 9], $data[$i - 8], $data[$i - 7], $data[$i - 6], $data[$i - 5], $data[$i - 4], $data[$i - 3], $data[$i - 2], $data[$i - 1], $data[$i]];
                $tmparr = $this->array2pack($tmparr);
                $tmparr = implode('', $tmparr);
                $e_block = $this->newKey($tmparr);
            }
    ...
    {
        return $this->decrypt(base64_decode((string) $data));
    }
    ...
    {
        if (!str_contains((string) $data, 'Qw3j0iA')) {
            return false;
        }
        
        [$rand, $data_crypt] = explode('Qw3j0iA', (string) $data);
        if (isset($data_crypt)) {
    ...
    function binmd5($string)
    {
        ...
        return pack('H*', md5((string) $string));
    }
