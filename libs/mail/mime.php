...
class MIME {
	...
	public $_qpvrep = [
			...
			"=FF"];
	public $_qpkstr = "";
	...
	function is_printable($str){
		return (strcspn((string) $str, (string) $this->_qpkstr) == strlen((string) $str));
	}

	function qpencode($str, $len = 74, $end = "\r\n"){

		$out = '';
        $exp = explode($end, (string) $str);
        foreach($exp as $line){
			$line = str_replace('=', '=3D', $line);
            $line = str_replace($this->_qpkeys, $this->_qpvrep, $line);
            preg_match_all('/.{1,'.$len.'}([^=]{0,2})?/', $line, $match);
            for($i = 0; $i < count($match[0]); $i++){
				$line = (str_ends_with($match[0][$i], " ")) ? substr($match[0][$i], 0, -1).'=20' : $match[0][$i];
                if(($i+1) < count($match[0])) $line .= '=';
                $out .= $line.$end;
            }
        }
        if($out == '') return '';
        return substr($out, 0, -(strlen((string) $end)));
