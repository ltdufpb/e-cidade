...
function submenus($item,$id,$mod) {
  ...
  if(pg_num_rows($sub) > 0) {
    ...
    for($x = 0;$x < pg_num_rows($sub);$x++) {

		  $valor = pg_fetch_result($sub,$x,"id_item")."#".pg_fetch_result($sub,$x,"id_item_filho")."#".pg_fetch_result($sub,$x,"modulo");
      echo "<img src=\"imagens/alinha.gif\" height=\"5\" id=\"Img".$conta."\" width=\"".$wid."\" ><input onClick=\"js_marcaP1('$id','Img".$conta."');js_marcaP2('$id','Img".$conta."')\" type=\"checkbox\" id=\"ID$valor\" name=\"CHECK$valor\" value=\"$valor\" onmouseover=\"js_msg_status('".pg_fetch_result($sub,$x,"help")."')\" onmouseout=\"js_lmp_status()\"><label onmouseover=\"js_msg_status('".pg_fetch_result($sub,$x,"help")."')\" onmouseout=\"js_lmp_status()\" for=\"ID$valor\">".pg_fetch_result($sub,$x,1)."</label><br>\n";
  	  $wid += 15;
	    $conta++;
...
$result = db_query("select i.id_item as pai,m.id_item,m.id_item_filho,m.modulo,i.descricao,i.help,i.funcao 
                     ...
                     inner join db_menu m on m.id_item_filho = i.id_item 
	                   where m.modulo = ".db_strpos($_POST["modulos"],"##")."
							         and m.id_item = ".db_strpos($_POST["modulos"],"##")."
							         and i.itemativo = $ambiente
                     order by {$sOrdenacao} asc");
echo "<table border=\"1\" cellpadding=\"10\" cellspacing=\"0\">\n<tr border=\"1\">\n";
for($i = 0;$i < pg_num_rows($result);$i++) {

	$valor = pg_fetch_result($result,$i,"id_item")."#".pg_fetch_result($result,$i,"id_item_filho")."#".pg_fetch_result($result,$i,"modulo");
  if($ambiente == "1") {

	  echo "<td id=\"col$i\" valign=\"top\" nowrap>\n<input onclick=\"js_marca('col$i',this)\" type=\"checkbox\" id=\"ID$valor\" name=\"CHECK$valor\" value=\"$valor\" onmouseover=\"js_msg_status('".pg_fetch_result($result,$i,"help")."')\" onmouseout=\"js_lmp_status()\"><label onmouseover=\"js_msg_status('".pg_fetch_result($result,$i,"help")."')\" onmouseout=\"js_lmp_status()\" for=\"ID$valor\">".pg_fetch_result($result,$i,"descricao")."</label><br>\n";
  } else {
