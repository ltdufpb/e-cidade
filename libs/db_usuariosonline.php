...
global $conn;
$result = db_query("select descricao from db_itensmenu where funcao = '".basename((string) $_SERVER['PHP_SELF'])."'");
if(pg_num_rows($result) > 0)
  $str = pg_fetch_result($result,0,0);
else
  $str = basename((string) $_SERVER['PHP_SELF']);
...
if(pg_num_rows($result) == 0) {
  ...
  db_query("insert into db_usuariosonline
    values(".db_getsession("DB_id_usuario").",
      ...
      ".time().")") or die("Erro:(27) inserindo arquivo em db_usuariosonline: ".pg_last_error());
