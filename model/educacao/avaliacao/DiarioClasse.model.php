...
class DiarioClasse
{
    ...
    public function getDisciplinas()
    {
        if (count($this->aDiarioDisciplina) == 0) {
            ...
            $sSqlDiarioClasse = $oDaoDiarioClasse->sql_query_diario_classe(
                null,
                'diario.*, ed59_i_codigo',
                "ed95_i_codigo",
                $sWhereDiario
            );
        ...
        if (!is_null($areaProcedimento)) {
            ...
            foreach ($regencias as $regencia) {
                ...
                if (is_null($areaConhecimento)) {
                    $msgErro = 'Procedimento da turma é avaliado por "Área de Conhecimento" porém existe disciplina na';
                    $msgErro .= 'turma sem "Área de Conhecimento" informada. Exemplo: ';
    ...
    private function atualizarResultado(DiarioAvaliacaoDisciplina $oDiarioDisciplina, ResultadoAvaliacao $oPeriodo)
    {
        ...
        /**
         * Se ainda não existe o resultado no diário do aluno, cria o diárioresultado e se o elemento gerar resultado
