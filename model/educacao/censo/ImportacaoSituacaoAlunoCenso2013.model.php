...
class ImportacaoSituacaoAlunoCenso2013 {
   ...
   * @param DBLogJSON $oLog
   * @param integer $iAnoArquivo ano do censo
   */
  public function __construct( /**
   * Caminho do arquivo
   */
  private $sCaminhoArquivo, private readonly DBLogJSON $oLog, /**
   * Ano do arquivo.
  ...
  public function log( $lErro, $sMensagem ) {
    ...
    $oMensagem->sMensagem = mb_convert_encoding( $sMensagem, 'UTF-8', 'ISO-8859-1' );
  ...
  public function validaDados( DBLayoutLinha $oLinha, $iLinha, $iRegistro ) {
     ...
     */
    $iColunas = explode( "|" , (string) $oLinha->getLinha() );
    ...
    foreach ( $oLinha->getProperties() as $sIndice => $aCampos ) {
       ...
       */
      if (    $sValor != null
           && strlen( (string) $sValor ) > $aCampos[1]
