 ...
 * Define as disciplinas que o o aluno est� cursando
...
final class ProgressaoParcialVinculoDisciplina {
  ...
  /**
   * Regencia em que  aluno est� vinculado
   * @var Regencia
   */
  private $oRegencia;
   ...
   * @param integer $iCodigoVinculo c�digo do vinculo
   */
  ...
  /**
   * Retorna a regencia em que a progressao est� vinculada
   * @return Regencia
   */
  ...
  public function salvar(ProgressaoParcialAluno $oProgressaoParcial) {
    ...
    if ($this->oRegencia == null) {
      throw new BusinessException("Reg�ncia n�o informada para a realizacao do vinculo.");
    }

    if ($this->dtVinculo == null) {
      throw new BusinessException("Data de vinculo n�o informado para a realizacao do vinculo.");
    }
    /**
    ...
    if ($this->iCodigoMatricula == null) {
      ...
      if ($oDaoProgressaoMatricula->erro_status == 0) {
        ...
        $sMsgErro = "Erro ao realizar vinculo da reg�ncia.\nErro T�cnico:\n{$oDaoProgressaoMatricula->erro_msg}";
    ...
    if (empty($this->iCodigoVinculo)) {
      ...
      if ($oDaoProgressaoMatricula->erro_status == 0) {
        ...
        $sMsgErro = "Erro ao salvar dados do vinculo da reg�ncia.\nErro T�cnico:\n{$oDaoProgressaoMatricula->erro_msg}";
        throw new BusinessException($sMsgErro);
      }
    }

    if ($oDaoProgressaoVinculo->erro_status == 0) {
      throw new BusinessException("Erro ao realizar vinculo da reg�ncia.\nErro T�cnico:\n{$oDaoProgressaoVinculo->erro_msg}");
    }
  }
   ...
   * novos v�nculos para dependencia, e remove tambem a matricula vinculada a dependencia
   */
