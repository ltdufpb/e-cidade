...
class RelatorioDiarioClasseBase extends PDF
{
     ...
     * @var integer
     */
    protected $filtrarAlunosMatriculadosTurnoReferencia;
    /**
     * @var string
     */
    private $nomeRegente;
    ...
    public function Header()
    {
        ...
        $turno = $this->oTurma->getTurno()->getDescricao();
        if (!is_null($this->filtrarAlunosMatriculadosTurnoReferencia)) {
            $turno = Turno::getDescricaoTurno($this->filtrarAlunosMatriculadosTurnoReferencia);
        }
        $this->Cell($iTamanhoLinha, $iAlturaLinha, "Disciplina: {$sDisciplina}", 0, 0, "L");
        $this->Cell($iTamanhoLinha, $iAlturaLinha, "Turno: {$turno}", 0, 1, "L");
        $this->SetX($iPosicaoX);
    ...
    private function organizarListaAlunos()
    {
        ...
        foreach ($this->estruturaSubCabecalho() as $iRegencia => $oEstrutura) {
            ...
            foreach ($aMatriculas as $iIndice => $oMatricula) {
                if (!is_null($this->filtrarAlunosMatriculadosTurnoReferencia)) {
                    if (!$oMatricula->estaMatriculadoNoTurnoReferente($this->filtrarAlunosMatriculadosTurnoReferencia)) {
                        continue;
                    }
                }
                ...
                if (!$this->validaSituacaoAluno($oAluno)) {
                    continue;
                }

                if ($alunosAdicionados % ($this->iNumeroAlunosPagina) == 0) {
                    $iPagina++;
                }
        ...
        foreach ($aAlunos as $aAlunosPagina) {
            ...
            foreach ($aAlunosPagina as $oDadosAluno) {
                ...
                if ($oDadosAluno->oMatricula->getSituacao() == 'MATRICULADO') {
                    ...
                    $oPeriodoAvaliacao = $this->oAvaliacaoPeriodica->getPeriodoAvaliacao();
                    $oPeriodoCalendario = $this->oTurma->getCalendario()->getPeriodoCalendarioPorPeriodoAvaliacao($oPeriodoAvaliacao);
                    $dataInicio = $oPeriodoCalendario->getDataInicio();
                    $dataFim = $oPeriodoCalendario->getDataTermino();
                    $dataEncerramento = $oDadosAluno->oMatricula->getDataEncerramento();

                    if (!$dataEncerramento->dataEstaNoIntervalo($dataEncerramento, $dataInicio, $dataFim) && $dataEncerramento < $dataInicio) {
                        $this->imprimeSituacaoAluno($oDadosAluno->oMatricula->getSituacao(), $oEstrutura->iTamanhoGrade);
                    } else {
                        $diaEncerramento = $dataEncerramento->getDia();
                        $mesEncerramento = $dataEncerramento->getMes();
                        $quantidadeCelulasParciais = $this->imprimirGradeFaltasAluno(
                            $oDadosAluno->aFaltas,
                            $oEstrutura,
                            $diaEncerramento,
                            $mesEncerramento
                        ...
                        $larguraCelulasRestantes = $oEstrutura->iLarguraCelulaGrade * $quantidadeCelulasParciais;
                        if ($larguraCelulasRestantes != 0) {
                            $quantidadeCaracteres = $quantidadeCelulasParciais + db_round($quantidadeCelulasParciais/2) - 1;
                            $situacao = substr((string) $oDadosAluno->oMatricula->getSituacao(), 0, $quantidadeCaracteres);
                            $this->imprimeSituacaoAluno($situacao, $larguraCelulasRestantes);
                        }
                    }
                }
     ...
     */
    private function imprimirGradeFaltasAluno($aFaltas, $oEstrutura, $diaEncerramento = null, $mesEncerramento = null)
    {
        ...
        if (count($oEstrutura->aMeses) == 0) {
            ...
            foreach ($oEstrutura->aMeses as $iMes => $oMes) {
                ...
                foreach ($oMes->aDias as $oDia) {
                    if (!empty($mesEncerramento) &&
                        !empty($diaEncerramento) &&
                        $iMes >= $mesEncerramento &&
                        $oDia->iDia >= $diaEncerramento) {
                        $encerramento = true;
                        continue;
                    }
                    $sFalta = "";
                    ...
                    $quantidadeCelulasImpressa++;
                }
            }

            $quantidadeCelulasFaltantes = $oEstrutura->iNumeroColunas - $quantidadeCelulasImpressa;
            if (!empty($diaEncerramento) && !empty($mesEncerramento)) {
                $quantidadeCelulasFaltantes += $oEstrutura->iNumeroColunasVazias;
            } else {
                $this->escreverColunasFaltasEmBranco($oEstrutura);
            }
            return $quantidadeCelulasFaltantes;
        }
    }
    ...
    public function filtrarAlunosMatriculadosTurnoReferencia($turnoRefente)
    {
        $this->filtrarAlunosMatriculadosTurnoReferencia = $turnoRefente;
    }

    /**
     * @param string $nomeRegente
     */
    public function setNomeRegente($nomeRegente = '')
    {
        $this->nomeRegente = $nomeRegente;
    }
}
