...
class cl_loteimpressaocartaoidentificacao { 
   ...
   public $ed305_sequencial = 0; 
   public $ed305_usuario = 0; 
   public $ed305_data_dia = null; 
   public $ed305_data_mes = null; 
   public $ed305_data_ano = null; 
   public $ed305_data = null; 
   public $ed305_hora = null; 
   ...
   function incluir ($ed305_sequencial){ 
     ...
     if($ed305_sequencial == "" || $ed305_sequencial == null ){
       ...
       $this->ed305_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from loteimpressaocartaoidentificacao_ed305_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $ed305_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,3344,18860,'','".AddSlashes(pg_fetch_result($resaco,0,'ed305_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3344,18861,'','".AddSlashes(pg_fetch_result($resaco,0,'ed305_usuario'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3344,18862,'','".AddSlashes(pg_fetch_result($resaco,0,'ed305_data'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3344,18863,'','".AddSlashes(pg_fetch_result($resaco,0,'ed305_hora'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($ed305_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->ed305_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ed305_sequencial"])){ 
       $sql  .= $virgula." ed305_sequencial = $this->ed305_sequencial ";
       $virgula = ",";
       if(trim((string) $this->ed305_sequencial) == null ){ 
     ...
     if(trim((string) $this->ed305_usuario)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ed305_usuario"])){ 
       $sql  .= $virgula." ed305_usuario = $this->ed305_usuario ";
       $virgula = ",";
       if(trim((string) $this->ed305_usuario) == null ){ 
         $this->erro_sql = " Campo Cod. Usuário nao Informado.";
         $this->erro_campo = "ed305_usuario";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->ed305_data)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ed305_data_dia"]) &&  ($GLOBALS["HTTP_POST_VARS"]["ed305_data_dia"] !="") ){ 
       $sql  .= $virgula." ed305_data = '$this->ed305_data' ";
       $virgula = ",";
       if(trim((string) $this->ed305_data) == null ){ 
         $this->erro_sql = " Campo Data nao Informado.";
         $this->erro_campo = "ed305_data_dia";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->ed305_hora)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ed305_hora"])){ 
       $sql  .= $virgula." ed305_hora = '$this->ed305_hora' ";
       $virgula = ",";
       if(trim((string) $this->ed305_hora) == null ){ 
         $this->erro_sql = " Campo Hora nao Informado.";
         $this->erro_campo = "ed305_hora";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["ed305_sequencial"]) || $this->ed305_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,3344,18860,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ed305_sequencial'))."','$this->ed305_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ed305_usuario"]) || $this->ed305_usuario != "")
           $resac = db_query("insert into db_acount values($acount,3344,18861,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ed305_usuario'))."','$this->ed305_usuario',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ed305_data"]) || $this->ed305_data != "")
           $resac = db_query("insert into db_acount values($acount,3344,18862,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ed305_data'))."','$this->ed305_data',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ed305_hora"]) || $this->ed305_hora != "")
           $resac = db_query("insert into db_acount values($acount,3344,18863,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ed305_hora'))."','$this->ed305_hora',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($ed305_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,3344,18860,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ed305_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3344,18861,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ed305_usuario'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3344,18862,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ed305_data'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3344,18863,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ed305_hora'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
