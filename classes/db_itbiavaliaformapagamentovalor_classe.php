...
class cl_itbiavaliaformapagamentovalor { 
   ...
   public $it24_sequencial = 0; 
   public $it24_itbitransacaoformapag = 0; 
   public $it24_itbiavalia = 0; 
   public $it24_valor = 0; 
   ...
   function incluir ($it24_sequencial){ 
     ...
     if($it24_sequencial == "" || $it24_sequencial == null ){
       ...
       $this->it24_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from itbiavaliaformapagamentovalor_it24_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $it24_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2360,13489,'','".AddSlashes(pg_fetch_result($resaco,0,'it24_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2360,13490,'','".AddSlashes(pg_fetch_result($resaco,0,'it24_itbitransacaoformapag'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2360,13491,'','".AddSlashes(pg_fetch_result($resaco,0,'it24_itbiavalia'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2360,13492,'','".AddSlashes(pg_fetch_result($resaco,0,'it24_valor'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($it24_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->it24_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["it24_sequencial"])){ 
       $sql  .= $virgula." it24_sequencial = $this->it24_sequencial ";
       $virgula = ",";
       if(trim((string) $this->it24_sequencial) == null ){ 
     ...
     if(trim((string) $this->it24_itbitransacaoformapag)!="" || isset($GLOBALS["HTTP_POST_VARS"]["it24_itbitransacaoformapag"])){ 
       $sql  .= $virgula." it24_itbitransacaoformapag = $this->it24_itbitransacaoformapag ";
       $virgula = ",";
       if(trim((string) $this->it24_itbitransacaoformapag) == null ){ 
         $this->erro_sql = " Campo Transação nao Informado.";
         $this->erro_campo = "it24_itbitransacaoformapag";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->it24_itbiavalia)!="" || isset($GLOBALS["HTTP_POST_VARS"]["it24_itbiavalia"])){ 
       $sql  .= $virgula." it24_itbiavalia = $this->it24_itbiavalia ";
       $virgula = ",";
       if(trim((string) $this->it24_itbiavalia) == null ){ 
         $this->erro_sql = " Campo Avalia nao Informado.";
         $this->erro_campo = "it24_itbiavalia";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->it24_valor)!="" || isset($GLOBALS["HTTP_POST_VARS"]["it24_valor"])){ 
       $sql  .= $virgula." it24_valor = $this->it24_valor ";
       $virgula = ",";
       if(trim((string) $this->it24_valor) == null ){ 
         $this->erro_sql = " Campo Valor nao Informado.";
         $this->erro_campo = "it24_valor";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["it24_sequencial"]))
           $resac = db_query("insert into db_acount values($acount,2360,13489,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'it24_sequencial'))."','$this->it24_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["it24_itbitransacaoformapag"]))
           $resac = db_query("insert into db_acount values($acount,2360,13490,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'it24_itbitransacaoformapag'))."','$this->it24_itbitransacaoformapag',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["it24_itbiavalia"]))
           $resac = db_query("insert into db_acount values($acount,2360,13491,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'it24_itbiavalia'))."','$this->it24_itbiavalia',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["it24_valor"]))
           $resac = db_query("insert into db_acount values($acount,2360,13492,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'it24_valor'))."','$this->it24_valor',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($it24_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2360,13489,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'it24_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2360,13490,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'it24_itbitransacaoformapag'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2360,13491,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'it24_itbiavalia'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2360,13492,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'it24_valor'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
