...
class cl_autofiscal { 
   ...
   function incluir ($y51_codauto){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,701,4995,'','".AddSlashes(pg_fetch_result($resaco,0,'y51_codauto'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,701,4996,'','".AddSlashes(pg_fetch_result($resaco,0,'y51_codnoti'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($y51_codauto=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->y51_codauto)!="" || isset($GLOBALS["HTTP_POST_VARS"]["y51_codauto"])){ 
       $sql  .= $virgula." y51_codauto = $this->y51_codauto ";
       $virgula = ",";
       if(trim((string) $this->y51_codauto) == null ){ 
     ...
     if(trim((string) $this->y51_codnoti)!="" || isset($GLOBALS["HTTP_POST_VARS"]["y51_codnoti"])){ 
       $sql  .= $virgula." y51_codnoti = $this->y51_codnoti ";
       $virgula = ",";
       if(trim((string) $this->y51_codnoti) == null ){ 
         $this->erro_sql = " Campo Código da Notificação nao Informado.";
         $this->erro_campo = "y51_codnoti";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["y51_codauto"]))
           $resac = db_query("insert into db_acount values($acount,701,4995,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'y51_codauto'))."','$this->y51_codauto',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["y51_codnoti"]))
           $resac = db_query("insert into db_acount values($acount,701,4996,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'y51_codnoti'))."','$this->y51_codnoti',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($y51_codauto=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,701,4995,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'y51_codauto'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,701,4996,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'y51_codnoti'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
