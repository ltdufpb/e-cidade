...
class cl_sau_prestadorvinculos { 
   ...
   public $s111_i_codigo = 0; 
   public $s111_i_prestador = 0; 
   public $s111_procedimento = 0; 
   public $s111_c_situacao = null; 
   ...
   function incluir ($s111_i_codigo){ 
     ...
     if($s111_i_codigo == "" || $s111_i_codigo == null ){
       ...
       $this->s111_i_codigo = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from sau_prestadorviinculos_codigo_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $s111_i_codigo)){
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,2374,13566,'','".AddSlashes(pg_fetch_result($resaco,0,'s111_i_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2374,13567,'','".AddSlashes(pg_fetch_result($resaco,0,'s111_i_prestador'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2374,13568,'','".AddSlashes(pg_fetch_result($resaco,0,'s111_procedimento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2374,13570,'','".AddSlashes(pg_fetch_result($resaco,0,'s111_c_situacao'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
   ...
   public function alterar ($s111_i_codigo=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->s111_i_codigo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s111_i_codigo"])){ 
       $sql  .= $virgula." s111_i_codigo = $this->s111_i_codigo ";
       $virgula = ",";
       if(trim((string) $this->s111_i_codigo) == null ){ 
     ...
     if(trim((string) $this->s111_i_prestador)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s111_i_prestador"])){ 
       $sql  .= $virgula." s111_i_prestador = $this->s111_i_prestador ";
       $virgula = ",";
       if(trim((string) $this->s111_i_prestador) == null ){ 
         $this->erro_sql = " Campo Prestador não informado.";
         $this->erro_campo = "s111_i_prestador";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->s111_procedimento)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s111_procedimento"])){ 
       $sql  .= $virgula." s111_procedimento = $this->s111_procedimento ";
       $virgula = ",";
       if(trim((string) $this->s111_procedimento) == null ){ 
         $this->erro_sql = " Campo Exame não informado.";
         $this->erro_campo = "s111_procedimento";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->s111_c_situacao)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s111_c_situacao"])){ 
       $sql  .= $virgula." s111_c_situacao = '$this->s111_c_situacao' ";
       $virgula = ",";
       if(trim((string) $this->s111_c_situacao) == null ){ 
         $this->erro_sql = " Campo Situação não informado.";
         $this->erro_campo = "s111_c_situacao";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["s111_i_codigo"]) || $this->s111_i_codigo != "")
             $resac = db_query("insert into db_acount values($acount,2374,13566,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'s111_i_codigo'))."','$this->s111_i_codigo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if (isset($GLOBALS["HTTP_POST_VARS"]["s111_i_prestador"]) || $this->s111_i_prestador != "")
             $resac = db_query("insert into db_acount values($acount,2374,13567,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'s111_i_prestador'))."','$this->s111_i_prestador',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if (isset($GLOBALS["HTTP_POST_VARS"]["s111_procedimento"]) || $this->s111_procedimento != "")
             $resac = db_query("insert into db_acount values($acount,2374,13568,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'s111_procedimento'))."','$this->s111_procedimento',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if (isset($GLOBALS["HTTP_POST_VARS"]["s111_c_situacao"]) || $this->s111_c_situacao != "")
             $resac = db_query("insert into db_acount values($acount,2374,13570,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'s111_c_situacao'))."','$this->s111_c_situacao',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,2374,13566,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'s111_i_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,2374,13567,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'s111_i_prestador'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,2374,13568,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'s111_procedimento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,2374,13570,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'s111_c_situacao'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
