...
class cl_rhtipoemp { 
   ...
   function incluir ($rh12_tipo){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,975,6067,'','".AddSlashes(pg_fetch_result($resaco,0,'rh12_tipo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,975,6068,'','".AddSlashes(pg_fetch_result($resaco,0,'rh12_descr'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($rh12_tipo=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->rh12_tipo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh12_tipo"])){ 
       $sql  .= $virgula." rh12_tipo = $this->rh12_tipo ";
       $virgula = ",";
       if(trim((string) $this->rh12_tipo) == null ){ 
     ...
     if(trim((string) $this->rh12_descr)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh12_descr"])){ 
       $sql  .= $virgula." rh12_descr = '$this->rh12_descr' ";
       $virgula = ",";
       if(trim((string) $this->rh12_descr) == null ){ 
         $this->erro_sql = " Campo Descrição do Tipo nao Informado.";
         $this->erro_campo = "rh12_descr";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["rh12_tipo"]))
           $resac = db_query("insert into db_acount values($acount,975,6067,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'rh12_tipo'))."','$this->rh12_tipo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["rh12_descr"]))
           $resac = db_query("insert into db_acount values($acount,975,6068,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'rh12_descr'))."','$this->rh12_descr',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($rh12_tipo=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,975,6067,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'rh12_tipo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,975,6068,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'rh12_descr'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
