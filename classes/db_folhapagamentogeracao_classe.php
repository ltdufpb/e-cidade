...
class cl_folhapagamentogeracao { 
   ...
   public $rh146_sequencial = 0; 
   public $rh146_folhapagamento = 0; 
   ...
   function incluir ($rh146_sequencial){ 
     ...
     if($rh146_sequencial == "" || $rh146_sequencial == null ){
       ...
       $this->rh146_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from folhapagamentogeracao_rh146_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $rh146_sequencial)){
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3749,20842,'','".AddSlashes(pg_fetch_result($resaco,0,'rh146_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3749,20843,'','".AddSlashes(pg_fetch_result($resaco,0,'rh146_folhapagamento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
   ...
   public function alterar ($rh146_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->rh146_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh146_sequencial"])){ 
       $sql  .= $virgula." rh146_sequencial = $this->rh146_sequencial ";
       $virgula = ",";
       if(trim((string) $this->rh146_sequencial) == null ){ 
     ...
     if(trim((string) $this->rh146_folhapagamento)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh146_folhapagamento"])){ 
       $sql  .= $virgula." rh146_folhapagamento = $this->rh146_folhapagamento ";
       $virgula = ",";
       if(trim((string) $this->rh146_folhapagamento) == null ){ 
         $this->erro_sql = " Campo Folha Pagamento não informado.";
         $this->erro_campo = "rh146_folhapagamento";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["rh146_sequencial"]) || $this->rh146_sequencial != "")
             $resac = db_query("insert into db_acount values($acount,3749,20842,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'rh146_sequencial'))."','$this->rh146_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if (isset($GLOBALS["HTTP_POST_VARS"]["rh146_folhapagamento"]) || $this->rh146_folhapagamento != "")
             $resac = db_query("insert into db_acount values($acount,3749,20843,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'rh146_folhapagamento'))."','$this->rh146_folhapagamento',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3749,20842,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'rh146_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,3749,20843,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'rh146_folhapagamento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
