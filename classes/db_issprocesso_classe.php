...
class cl_issprocesso { 
   ...
   function incluir ($q14_inscr){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,45,283,'','".AddSlashes(pg_fetch_result($resaco,0,'q14_inscr'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,45,284,'','".AddSlashes(pg_fetch_result($resaco,0,'q14_proces'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($q14_inscr=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->q14_inscr)!="" || isset($GLOBALS["HTTP_POST_VARS"]["q14_inscr"])){ 
       $sql  .= $virgula." q14_inscr = $this->q14_inscr ";
       $virgula = ",";
       if(trim((string) $this->q14_inscr) == null ){ 
     ...
     if(trim((string) $this->q14_proces)!="" || isset($GLOBALS["HTTP_POST_VARS"]["q14_proces"])){ 
       $sql  .= $virgula." q14_proces = $this->q14_proces ";
       $virgula = ",";
       if(trim((string) $this->q14_proces) == null ){ 
         $this->erro_sql = " Campo codigo do processo nao Informado.";
         $this->erro_campo = "q14_proces";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["q14_inscr"]))
           $resac = db_query("insert into db_acount values($acount,45,283,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'q14_inscr'))."','$this->q14_inscr',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["q14_proces"]))
           $resac = db_query("insert into db_acount values($acount,45,284,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'q14_proces'))."','$this->q14_proces',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($q14_inscr=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,45,283,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'q14_inscr'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,45,284,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'q14_proces'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
