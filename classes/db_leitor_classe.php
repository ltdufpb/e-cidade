...
class cl_leitor { 
   ...
   function incluir ($bi10_codigo){ 
     ...
     if($bi10_codigo == "" || $bi10_codigo == null ){
       ...
       $this->bi10_codigo = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from leitor_bi10_codigo_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $bi10_codigo)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,1008018,1008139,'','".AddSlashes(pg_fetch_result($resaco,0,'bi10_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($bi10_codigo=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->bi10_codigo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["bi10_codigo"])){ 
       $sql  .= $virgula." bi10_codigo = $this->bi10_codigo ";
       $virgula = ",";
       if(trim((string) $this->bi10_codigo) == null ){ 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["bi10_codigo"]))
           $resac = db_query("insert into db_acount values($acount,1008018,1008139,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'bi10_codigo'))."','$this->bi10_codigo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($bi10_codigo=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,1008018,1008139,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'bi10_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
