...
class cl_aguacondominio { 
   ...
   function incluir ($x31_codcondominio){ 
     ...
     if($x31_codcondominio == "" || $x31_codcondominio == null ){
       ...
       $this->x31_codcondominio = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from aguacondominio_x32_codcondominio_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $x31_codcondominio)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,1431,8429,'','".AddSlashes(pg_fetch_result($resaco,0,'x31_matric'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,1431,8440,'','".AddSlashes(pg_fetch_result($resaco,0,'x31_codcondominio'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($x31_codcondominio=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->x31_matric)!="" || isset($GLOBALS["HTTP_POST_VARS"]["x31_matric"])){ 
       $sql  .= $virgula." x31_matric = $this->x31_matric ";
       $virgula = ",";
       if(trim((string) $this->x31_matric) == null ){ 
         $this->erro_sql = " Campo Matrícula nao Informado.";
         $this->erro_campo = "x31_matric";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->x31_codcondominio)!="" || isset($GLOBALS["HTTP_POST_VARS"]["x31_codcondominio"])){ 
       $sql  .= $virgula." x31_codcondominio = $this->x31_codcondominio ";
       $virgula = ",";
       if(trim((string) $this->x31_codcondominio) == null ){ 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["x31_matric"]))
           $resac = db_query("insert into db_acount values($acount,1431,8429,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'x31_matric'))."','$this->x31_matric',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["x31_codcondominio"]))
           $resac = db_query("insert into db_acount values($acount,1431,8440,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'x31_codcondominio'))."','$this->x31_codcondominio',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($x31_codcondominio=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,1431,8429,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'x31_matric'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,1431,8440,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'x31_codcondominio'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
