...
class cl_habittipogrupoprogramaformaavaliacao { 
   ...
   public $ht06_sequencial = 0; 
   public $ht06_habitformaavaliacao = 0; 
   public $ht06_habittipogrupoprograma = 0; 
   ...
   function incluir ($ht06_sequencial){ 
     ...
     if($ht06_sequencial == "" || $ht06_sequencial == null ){
       ...
       $this->ht06_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from habittipogrupoprogramaformaavaliacao_ht06_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $ht06_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2994,16981,'','".AddSlashes(pg_fetch_result($resaco,0,'ht06_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2994,16982,'','".AddSlashes(pg_fetch_result($resaco,0,'ht06_habitformaavaliacao'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2994,16983,'','".AddSlashes(pg_fetch_result($resaco,0,'ht06_habittipogrupoprograma'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($ht06_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->ht06_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ht06_sequencial"])){ 
       $sql  .= $virgula." ht06_sequencial = $this->ht06_sequencial ";
       $virgula = ",";
       if(trim((string) $this->ht06_sequencial) == null ){ 
     ...
     if(trim((string) $this->ht06_habitformaavaliacao)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ht06_habitformaavaliacao"])){ 
       $sql  .= $virgula." ht06_habitformaavaliacao = $this->ht06_habitformaavaliacao ";
       $virgula = ",";
       if(trim((string) $this->ht06_habitformaavaliacao) == null ){ 
         $this->erro_sql = " Campo Forma de Avaliação nao Informado.";
         $this->erro_campo = "ht06_habitformaavaliacao";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->ht06_habittipogrupoprograma)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ht06_habittipogrupoprograma"])){ 
       $sql  .= $virgula." ht06_habittipogrupoprograma = $this->ht06_habittipogrupoprograma ";
       $virgula = ",";
       if(trim((string) $this->ht06_habittipogrupoprograma) == null ){ 
         $this->erro_sql = " Campo Tipo de Grupo nao Informado.";
         $this->erro_campo = "ht06_habittipogrupoprograma";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["ht06_sequencial"]) || $this->ht06_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,2994,16981,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ht06_sequencial'))."','$this->ht06_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ht06_habitformaavaliacao"]) || $this->ht06_habitformaavaliacao != "")
           $resac = db_query("insert into db_acount values($acount,2994,16982,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ht06_habitformaavaliacao'))."','$this->ht06_habitformaavaliacao',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ht06_habittipogrupoprograma"]) || $this->ht06_habittipogrupoprograma != "")
           $resac = db_query("insert into db_acount values($acount,2994,16983,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'ht06_habittipogrupoprograma'))."','$this->ht06_habittipogrupoprograma',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($ht06_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2994,16981,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ht06_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2994,16982,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ht06_habitformaavaliacao'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2994,16983,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'ht06_habittipogrupoprograma'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
