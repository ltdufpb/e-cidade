...
class cl_cadimobil { 
   ...
   function incluir ($j63_numcgm){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,394,2431,'','".AddSlashes(pg_fetch_result($resaco,0,'j63_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($j63_numcgm=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->j63_numcgm)!="" || isset($GLOBALS["HTTP_POST_VARS"]["j63_numcgm"])){ 
       $sql  .= $virgula." j63_numcgm = $this->j63_numcgm ";
       $virgula = ",";
       if(trim((string) $this->j63_numcgm) == null ){ 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["j63_numcgm"]))
           $resac = db_query("insert into db_acount values($acount,394,2431,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'j63_numcgm'))."','$this->j63_numcgm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($j63_numcgm=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,394,2431,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'j63_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
