...
class cl_aguabaseresp { 
   ...
   function incluir ($x14_matric){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,1427,8420,'','".AddSlashes(pg_fetch_result($resaco,0,'x14_matric'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,1427,8421,'','".AddSlashes(pg_fetch_result($resaco,0,'x14_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($x14_matric=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->x14_matric)!="" || isset($GLOBALS["HTTP_POST_VARS"]["x14_matric"])){ 
       $sql  .= $virgula." x14_matric = $this->x14_matric ";
       $virgula = ",";
       if(trim((string) $this->x14_matric) == null ){ 
     ...
     if(trim((string) $this->x14_numcgm)!="" || isset($GLOBALS["HTTP_POST_VARS"]["x14_numcgm"])){ 
       $sql  .= $virgula." x14_numcgm = $this->x14_numcgm ";
       $virgula = ",";
       if(trim((string) $this->x14_numcgm) == null ){ 
         $this->erro_sql = " Campo Responsável nao Informado.";
         $this->erro_campo = "x14_numcgm";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["x14_matric"]))
           $resac = db_query("insert into db_acount values($acount,1427,8420,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'x14_matric'))."','$this->x14_matric',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["x14_numcgm"]))
           $resac = db_query("insert into db_acount values($acount,1427,8421,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'x14_numcgm'))."','$this->x14_numcgm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($x14_matric=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,1427,8420,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'x14_matric'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,1427,8421,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'x14_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
