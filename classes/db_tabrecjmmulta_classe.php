...
class cl_tabrecjmmulta { 
   ...
   public $k140_sequencial = 0; 
   public $k140_tabrecjm = 0; 
   public $k140_multa = 0; 
   public $k140_faixa = 0; 
   ...
   function incluir ($k140_sequencial){ 
     ...
     if($k140_sequencial == "" || $k140_sequencial == null ){
       ...
       $this->k140_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from tabrecjmmulta_k140_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $k140_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,3366,18929,'','".AddSlashes(pg_fetch_result($resaco,0,'k140_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3366,18930,'','".AddSlashes(pg_fetch_result($resaco,0,'k140_tabrecjm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3366,18931,'','".AddSlashes(pg_fetch_result($resaco,0,'k140_multa'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3366,18932,'','".AddSlashes(pg_fetch_result($resaco,0,'k140_faixa'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($k140_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->k140_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["k140_sequencial"])){ 
       $sql  .= $virgula." k140_sequencial = $this->k140_sequencial ";
       $virgula = ",";
       if(trim((string) $this->k140_sequencial) == null ){ 
     ...
     if(trim((string) $this->k140_tabrecjm)!="" || isset($GLOBALS["HTTP_POST_VARS"]["k140_tabrecjm"])){ 
       $sql  .= $virgula." k140_tabrecjm = $this->k140_tabrecjm ";
       $virgula = ",";
       if(trim((string) $this->k140_tabrecjm) == null ){ 
         $this->erro_sql = " Campo Código do Juro e Multa nao Informado.";
         $this->erro_campo = "k140_tabrecjm";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->k140_multa)!="" || isset($GLOBALS["HTTP_POST_VARS"]["k140_multa"])){ 
       $sql  .= $virgula." k140_multa = $this->k140_multa ";
       $virgula = ",";
       if(trim((string) $this->k140_multa) == null ){ 
         $this->erro_sql = " Campo Dias Faixa nao Informado.";
         $this->erro_campo = "k140_multa";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->k140_faixa)!="" || isset($GLOBALS["HTTP_POST_VARS"]["k140_faixa"])){ 
       $sql  .= $virgula." k140_faixa = $this->k140_faixa ";
       $virgula = ",";
       if(trim((string) $this->k140_faixa) == null ){ 
         $this->erro_sql = " Campo Faixa Percentual nao Informado.";
         $this->erro_campo = "k140_faixa";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["k140_sequencial"]) || $this->k140_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,3366,18929,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'k140_sequencial'))."','$this->k140_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["k140_tabrecjm"]) || $this->k140_tabrecjm != "")
           $resac = db_query("insert into db_acount values($acount,3366,18930,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'k140_tabrecjm'))."','$this->k140_tabrecjm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["k140_multa"]) || $this->k140_multa != "")
           $resac = db_query("insert into db_acount values($acount,3366,18931,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'k140_multa'))."','$this->k140_multa',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["k140_faixa"]) || $this->k140_faixa != "")
           $resac = db_query("insert into db_acount values($acount,3366,18932,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'k140_faixa'))."','$this->k140_faixa',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($k140_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,3366,18929,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'k140_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3366,18930,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'k140_tabrecjm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3366,18931,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'k140_multa'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3366,18932,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'k140_faixa'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
