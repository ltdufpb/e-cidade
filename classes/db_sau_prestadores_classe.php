...
class cl_sau_prestadores {
   ...
   public $s110_i_codigo = 0;
   public $s110_i_numcgm = 0;
   ...
   function incluir ($s110_i_codigo){
     ...
     if($s110_i_codigo == "" || $s110_i_codigo == null ){
       ...
       $this->s110_i_codigo = pg_fetch_result($result,0,0);
     }else{
       $result = db_query("select last_value from sau_prestador_codigo_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $s110_i_codigo)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2373,13562,'','".AddSlashes(pg_fetch_result($resaco,0,'s110_i_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2373,13563,'','".AddSlashes(pg_fetch_result($resaco,0,'s110_i_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   }
   ...
   function alterar ($s110_i_codigo=null) {
     ...
     $virgula = "";
     if(trim((string) $this->s110_i_codigo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s110_i_codigo"])){
       $sql  .= $virgula." s110_i_codigo = $this->s110_i_codigo ";
       $virgula = ",";
       if(trim((string) $this->s110_i_codigo) == null ){
     ...
     if(trim((string) $this->s110_i_numcgm)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s110_i_numcgm"])){
       $sql  .= $virgula." s110_i_numcgm = $this->s110_i_numcgm ";
       $virgula = ",";
       if(trim((string) $this->s110_i_numcgm) == null ){
         $this->erro_sql = " Campo CGM nao Informado.";
         $this->erro_campo = "s110_i_numcgm";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["s110_i_codigo"]))
           $resac = db_query("insert into db_acount values($acount,2373,13562,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'s110_i_codigo'))."','$this->s110_i_codigo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["s110_i_numcgm"]))
           $resac = db_query("insert into db_acount values($acount,2373,13563,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'s110_i_numcgm'))."','$this->s110_i_numcgm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){
   ...
   function excluir ($s110_i_codigo=null,$dbwhere=null) {
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2373,13562,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'s110_i_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2373,13563,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'s110_i_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
