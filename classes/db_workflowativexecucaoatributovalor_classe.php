...
class cl_workflowativexecucaoatributovalor { 
   ...
   public $db111_sequencial = 0; 
   public $db111_workflowativexec = 0; 
   public $db111_cadattdinamicovalorgrupo = 0; 
   ...
   function incluir ($db111_sequencial){ 
     ...
     if($db111_sequencial == "" || $db111_sequencial == null ){
       ...
       $this->db111_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from workflowativexecucaoatributovalor_db111_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $db111_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,3157,17873,'','".AddSlashes(pg_fetch_result($resaco,0,'db111_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3157,17874,'','".AddSlashes(pg_fetch_result($resaco,0,'db111_workflowativexec'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3157,17906,'','".AddSlashes(pg_fetch_result($resaco,0,'db111_cadattdinamicovalorgrupo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($db111_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->db111_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["db111_sequencial"])){ 
       $sql  .= $virgula." db111_sequencial = $this->db111_sequencial ";
       $virgula = ",";
       if(trim((string) $this->db111_sequencial) == null ){ 
     ...
     if(trim((string) $this->db111_workflowativexec)!="" || isset($GLOBALS["HTTP_POST_VARS"]["db111_workflowativexec"])){ 
       $sql  .= $virgula." db111_workflowativexec = $this->db111_workflowativexec ";
       $virgula = ",";
       if(trim((string) $this->db111_workflowativexec) == null ){ 
         $this->erro_sql = " Campo Código Work Flow Atividade Execução nao Informado.";
         $this->erro_campo = "db111_workflowativexec";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->db111_cadattdinamicovalorgrupo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["db111_cadattdinamicovalorgrupo"])){ 
       $sql  .= $virgula." db111_cadattdinamicovalorgrupo = $this->db111_cadattdinamicovalorgrupo ";
       $virgula = ",";
       if(trim((string) $this->db111_cadattdinamicovalorgrupo) == null ){ 
         $this->erro_sql = " Campo Grupo de Valores de um Atributo nao Informado.";
         $this->erro_campo = "db111_cadattdinamicovalorgrupo";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["db111_sequencial"]) || $this->db111_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,3157,17873,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'db111_sequencial'))."','$this->db111_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["db111_workflowativexec"]) || $this->db111_workflowativexec != "")
           $resac = db_query("insert into db_acount values($acount,3157,17874,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'db111_workflowativexec'))."','$this->db111_workflowativexec',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["db111_cadattdinamicovalorgrupo"]) || $this->db111_cadattdinamicovalorgrupo != "")
           $resac = db_query("insert into db_acount values($acount,3157,17906,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'db111_cadattdinamicovalorgrupo'))."','$this->db111_cadattdinamicovalorgrupo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($db111_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,3157,17873,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'db111_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3157,17874,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'db111_workflowativexec'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3157,17906,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'db111_cadattdinamicovalorgrupo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
