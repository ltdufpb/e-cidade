...
class cl_fandamvenc { 
   ...
   function incluir ($y44_codandam){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,697,4980,'','".AddSlashes(pg_fetch_result($resaco,0,'y44_codandam'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,697,4981,'','".AddSlashes(pg_fetch_result($resaco,0,'y44_dtvenc'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($y44_codandam=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->y44_codandam)!="" || isset($GLOBALS["HTTP_POST_VARS"]["y44_codandam"])){ 
       $sql  .= $virgula." y44_codandam = $this->y44_codandam ";
       $virgula = ",";
       if(trim((string) $this->y44_codandam) == null ){ 
     ...
     if(trim((string) $this->y44_dtvenc)!="" || isset($GLOBALS["HTTP_POST_VARS"]["y44_dtvenc_dia"]) &&  ($GLOBALS["HTTP_POST_VARS"]["y44_dtvenc_dia"] !="") ){ 
       $sql  .= $virgula." y44_dtvenc = '$this->y44_dtvenc' ";
       $virgula = ",";
       if(trim((string) $this->y44_dtvenc) == null ){ 
         $this->erro_sql = " Campo Data de Vencimento nao Informado.";
         $this->erro_campo = "y44_dtvenc_dia";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["y44_codandam"]))
           $resac = db_query("insert into db_acount values($acount,697,4980,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'y44_codandam'))."','$this->y44_codandam',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["y44_dtvenc"]))
           $resac = db_query("insert into db_acount values($acount,697,4981,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'y44_dtvenc'))."','$this->y44_dtvenc',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($y44_codandam=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,697,4980,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'y44_codandam'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,697,4981,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'y44_dtvenc'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
