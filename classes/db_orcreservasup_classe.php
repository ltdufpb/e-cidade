...
class cl_orcreservasup { 
   ...
   function incluir ($o81_codres){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,776,5319,'','".AddSlashes(pg_fetch_result($resaco,0,'o81_codres'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,776,5320,'','".AddSlashes(pg_fetch_result($resaco,0,'o81_codsup'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($o81_codres=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->o81_codres)!="" || isset($GLOBALS["HTTP_POST_VARS"]["o81_codres"])){ 
       $sql  .= $virgula." o81_codres = $this->o81_codres ";
       $virgula = ",";
       if(trim((string) $this->o81_codres) == null ){ 
     ...
     if(trim((string) $this->o81_codsup)!="" || isset($GLOBALS["HTTP_POST_VARS"]["o81_codsup"])){ 
       $sql  .= $virgula." o81_codsup = $this->o81_codsup ";
       $virgula = ",";
       if(trim((string) $this->o81_codsup) == null ){ 
         $this->erro_sql = " Campo Código Suplementação nao Informado.";
         $this->erro_campo = "o81_codsup";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["o81_codres"]))
           $resac = db_query("insert into db_acount values($acount,776,5319,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'o81_codres'))."','$this->o81_codres',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["o81_codsup"]))
           $resac = db_query("insert into db_acount values($acount,776,5320,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'o81_codsup'))."','$this->o81_codsup',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($o81_codres=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,776,5319,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'o81_codres'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,776,5320,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'o81_codsup'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
