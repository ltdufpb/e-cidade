...
class cl_db_syscampodep { 
   ...
   function incluir ($codcam){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,397,752,'','".AddSlashes(pg_fetch_result($resaco,0,'codcam'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,397,2437,'','".AddSlashes(pg_fetch_result($resaco,0,'codcampai'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($codcam=null) { 
     ...
     if(trim((string) $this->codcampai)!="" || isset($GLOBALS["HTTP_POST_VARS"]["codcampai"])){ 
       $sql  .= $virgula." codcampai = $this->codcampai ";
       $virgula = ",";
       if(trim((string) $this->codcampai) == null ){ 
         $this->erro_sql = " Campo Código Campo Pai nao Informado.";
         $this->erro_campo = "codcampai";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["codcam"]))
           $resac = db_query("insert into db_acount values($acount,397,752,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'codcam'))."','$this->codcam',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["codcampai"]))
           $resac = db_query("insert into db_acount values($acount,397,2437,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'codcampai'))."','$this->codcampai',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($codcam=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,397,752,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'codcam'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,397,2437,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'codcampai'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
