...
class cl_configuracaodesdobramentopatrimonio {
   ...
   public $e135_sequencial = 0;
   public $e135_desdobramento = 0;
   ...
   function incluir ($e135_sequencial){
     ...
     if($e135_sequencial == "" || $e135_sequencial == null ){
       ...
       $this->e135_sequencial = pg_fetch_result($result,0,0);
     }else{
       $result = db_query("select last_value from configuracaodesdobramentopatrimonio_e135_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $e135_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,3348,18885,'','".AddSlashes(pg_fetch_result($resaco,0,'e135_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3348,18886,'','".AddSlashes(pg_fetch_result($resaco,0,'e135_desdobramento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   }
   ...
   function alterar ($e135_sequencial=null) {
     ...
     $virgula = "";
     if(trim((string) $this->e135_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["e135_sequencial"])){
       $sql  .= $virgula." e135_sequencial = $this->e135_sequencial ";
       $virgula = ",";
       if(trim((string) $this->e135_sequencial) == null ){
     ...
     if(trim((string) $this->e135_desdobramento)!="" || isset($GLOBALS["HTTP_POST_VARS"]["e135_desdobramento"])){
       $sql  .= $virgula." e135_desdobramento = $this->e135_desdobramento ";
       $virgula = ",";
       if(trim((string) $this->e135_desdobramento) == null ){
         $this->erro_sql = " Campo Desdobramento nao Informado.";
         $this->erro_campo = "e135_desdobramento";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["e135_sequencial"]) || $this->e135_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,3348,18885,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'e135_sequencial'))."','$this->e135_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["e135_desdobramento"]) || $this->e135_desdobramento != "")
           $resac = db_query("insert into db_acount values($acount,3348,18886,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'e135_desdobramento'))."','$this->e135_desdobramento',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){
   ...
   function excluir ($e135_sequencial=null,$dbwhere=null) {
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,3348,18885,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'e135_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3348,18886,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'e135_desdobramento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
