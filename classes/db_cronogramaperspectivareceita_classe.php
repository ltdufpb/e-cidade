...
class cl_cronogramaperspectivareceita { 
   ...
   public $o126_cronogramaperspectiva = 0; 
   ...
   function incluir ($o126_sequencial){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2620,14887,'','".AddSlashes(pg_fetch_result($resaco,0,'o126_cronogramaperspectiva'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($o126_sequencial=null) { 
     ...
     if(trim((string) $this->o126_cronogramaperspectiva)!="" || isset($GLOBALS["HTTP_POST_VARS"]["o126_cronogramaperspectiva"])){ 
       $sql  .= $virgula." o126_cronogramaperspectiva = $this->o126_cronogramaperspectiva ";
       $virgula = ",";
       if(trim((string) $this->o126_cronogramaperspectiva) == null ){ 
         $this->erro_sql = " Campo Perspectiva do Cronograma nao Informado.";
         $this->erro_campo = "o126_cronogramaperspectiva";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["o126_cronogramaperspectiva"]) || $this->o126_cronogramaperspectiva != "")
           $resac = db_query("insert into db_acount values($acount,2620,14887,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'o126_cronogramaperspectiva'))."','$this->o126_cronogramaperspectiva',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($o126_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2620,14887,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'o126_cronogramaperspectiva'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
...
function sql_query_receitaplano( $o126_sequencial=null,$campos="*",$ordem=null,$dbwhere="", $iPerspectiva = null){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = preg_split("#\\##m",$campos);
       $virgula = "";
