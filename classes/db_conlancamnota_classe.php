...
class cl_conlancamnota {
    ...
    function incluir ($c66_codlan,$c66_codnota){
        ...
        if(($resaco!=false)||($this->numrows!=0)){
            ...
            $resac = db_query("insert into db_acount values($acount,1147,6945,'','".AddSlashes(pg_fetch_result($resaco,0,'c66_codlan'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
            $resac = db_query("insert into db_acount values($acount,1147,6946,'','".AddSlashes(pg_fetch_result($resaco,0,'c66_codnota'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
        }
        return true;
    }
    ...
    function alterar ($c66_codlan=null,$c66_codnota=null) {
        ...
        $virgula = "";
        if(trim((string) $this->c66_codlan)!="" || isset($GLOBALS["HTTP_POST_VARS"]["c66_codlan"])){
            $sql  .= $virgula." c66_codlan = $this->c66_codlan ";
            $virgula = ",";
            if(trim((string) $this->c66_codlan) == null ){
        ...
        if(trim((string) $this->c66_codnota)!="" || isset($GLOBALS["HTTP_POST_VARS"]["c66_codnota"])){
            $sql  .= $virgula." c66_codnota = $this->c66_codnota ";
            $virgula = ",";
            if(trim((string) $this->c66_codnota) == null ){
        ...
        if($this->numrows>0){
            for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
                ...
                if(isset($GLOBALS["HTTP_POST_VARS"]["c66_codlan"]))
                    $resac = db_query("insert into db_acount values($acount,1147,6945,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'c66_codlan'))."','$this->c66_codlan',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
                if(isset($GLOBALS["HTTP_POST_VARS"]["c66_codnota"]))
                    $resac = db_query("insert into db_acount values($acount,1147,6946,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'c66_codnota'))."','$this->c66_codnota',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
            }
        }
        $result = db_query($sql);
        if($result==false){
    ...
    function excluir ($c66_codlan=null,$c66_codnota=null,$dbwhere=null) {
        ...
        if(($resaco!=false)||($this->numrows!=0)){
            for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
                ...
                $resac = db_query("insert into db_acount values($acount,1147,6945,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'c66_codlan'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
                $resac = db_query("insert into db_acount values($acount,1147,6946,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'c66_codnota'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
            }
        }
     ...
     * @return string
     */
    public function sqlProgramacaoFinanceiraParcela($aCampos = [], $aWhere = [], $aOrder = []) {
