...
class cl_rhgeracaofolhaarquivoreg { 
   ...
   public $rh106_sequencial = 0; 
   public $rh106_rhgeracaofolhaarquivo = 0; 
   public $rh106_rhgeracaofolhareg = 0; 
   ...
   function incluir ($rh106_sequencial){ 
     ...
     if($rh106_sequencial == "" || $rh106_sequencial == null ){
       ...
       $this->rh106_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = @db_query("select last_value from rhgeracaofolhaarquivoreg_rh106_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $rh106_sequencial)){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,3202,18143,'','".pg_fetch_result($resaco,0,'rh106_sequencial')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3202,18144,'','".pg_fetch_result($resaco,0,'rh106_rhgeracaofolhaarquivo')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,3202,18145,'','".pg_fetch_result($resaco,0,'rh106_rhgeracaofolhareg')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($rh106_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->rh106_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh106_sequencial"])){ 
        if(trim((string) $this->rh106_sequencial)=="" && isset($GLOBALS["HTTP_POST_VARS"]["rh106_sequencial"])){ 
           $this->rh106_sequencial = "0" ; 
        } 
       $sql  .= $virgula." rh106_sequencial = $this->rh106_sequencial ";
       $virgula = ",";
       if(trim((string) $this->rh106_sequencial) == null ){ 
     ...
     if(trim((string) $this->rh106_rhgeracaofolhaarquivo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh106_rhgeracaofolhaarquivo"])){ 
        if(trim((string) $this->rh106_rhgeracaofolhaarquivo)=="" && isset($GLOBALS["HTTP_POST_VARS"]["rh106_rhgeracaofolhaarquivo"])){ 
           $this->rh106_rhgeracaofolhaarquivo = "0" ; 
        } 
       $sql  .= $virgula." rh106_rhgeracaofolhaarquivo = $this->rh106_rhgeracaofolhaarquivo ";
       $virgula = ",";
       if(trim((string) $this->rh106_rhgeracaofolhaarquivo) == null ){ 
         $this->erro_sql = " Campo Código da Geração do Arquivo nao Informado.";
         $this->erro_campo = "rh106_rhgeracaofolhaarquivo";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->rh106_rhgeracaofolhareg)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh106_rhgeracaofolhareg"])){ 
        if(trim((string) $this->rh106_rhgeracaofolhareg)=="" && isset($GLOBALS["HTTP_POST_VARS"]["rh106_rhgeracaofolhareg"])){ 
           $this->rh106_rhgeracaofolhareg = "0" ; 
        } 
       $sql  .= $virgula." rh106_rhgeracaofolhareg = $this->rh106_rhgeracaofolhareg ";
       $virgula = ",";
       if(trim((string) $this->rh106_rhgeracaofolhareg) == null ){ 
         $this->erro_sql = " Campo Código do registro da geração nao Informado.";
         $this->erro_campo = "rh106_rhgeracaofolhareg";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
...
";
     ...
     if($this->numrows>0){       $resac = db_query("select nextval('db_acount_id_acount_seq') as acount");
       ...
       if(isset($GLOBALS["HTTP_POST_VARS"]["rh106_sequencial"]))
         $resac = db_query("insert into db_acount values($acount,3202,18143,'".pg_fetch_result($resaco,0,'rh106_sequencial')."','$this->rh106_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["rh106_rhgeracaofolhaarquivo"]))
         $resac = db_query("insert into db_acount values($acount,3202,18144,'".pg_fetch_result($resaco,0,'rh106_rhgeracaofolhaarquivo')."','$this->rh106_rhgeracaofolhaarquivo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["rh106_rhgeracaofolhareg"]))
         $resac = db_query("insert into db_acount values($acount,3202,18145,'".pg_fetch_result($resaco,0,'rh106_rhgeracaofolhareg')."','$this->rh106_rhgeracaofolhareg',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     $result = @db_query($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
