...
class cl_itinerariologradouro { 
   ...
   public $tre10_sequencial = 0; 
   public $tre10_linhatransporteitinerario = 0; 
   public $tre10_cadenderbairrocadenderrua = 0; 
   public $tre10_ordem = 0; 
   ...
   function incluir ($tre10_sequencial){ 
     ...
     if($tre10_sequencial == "" || $tre10_sequencial == null ){
       ...
       $this->tre10_sequencial = pg_fetch_result($result,0,0); 
     }else{
       $result = db_query("select last_value from itinerariologradouro_tre10_sequencial_seq");
       if(($result != false) && (pg_fetch_result($result,0,0) < $tre10_sequencial)){
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3607,20106,'','".AddSlashes(pg_fetch_result($resaco,0,'tre10_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3607,20107,'','".AddSlashes(pg_fetch_result($resaco,0,'tre10_linhatransporteitinerario'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3607,20108,'','".AddSlashes(pg_fetch_result($resaco,0,'tre10_cadenderbairrocadenderrua'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3607,20109,'','".AddSlashes(pg_fetch_result($resaco,0,'tre10_ordem'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
   ...
   function alterar ($tre10_sequencial=null) { 
     ...
     $virgula = "";
     if(trim((string) $this->tre10_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["tre10_sequencial"])){ 
       $sql  .= $virgula." tre10_sequencial = $this->tre10_sequencial ";
       $virgula = ",";
       if(trim((string) $this->tre10_sequencial) == null ){ 
         $this->erro_sql = " Campo Sequencial nao Informado.";
     ...
     if(trim((string) $this->tre10_linhatransporteitinerario)!="" || isset($GLOBALS["HTTP_POST_VARS"]["tre10_linhatransporteitinerario"])){ 
       $sql  .= $virgula." tre10_linhatransporteitinerario = $this->tre10_linhatransporteitinerario ";
       $virgula = ",";
       if(trim((string) $this->tre10_linhatransporteitinerario) == null ){ 
         $this->erro_sql = " Campo Sequencial nao Informado.";
         $this->erro_campo = "tre10_linhatransporteitinerario";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->tre10_cadenderbairrocadenderrua)!="" || isset($GLOBALS["HTTP_POST_VARS"]["tre10_cadenderbairrocadenderrua"])){ 
       $sql  .= $virgula." tre10_cadenderbairrocadenderrua = $this->tre10_cadenderbairrocadenderrua ";
       $virgula = ",";
       if(trim((string) $this->tre10_cadenderbairrocadenderrua) == null ){ 
         $this->erro_sql = " Campo Sequencial nao Informado.";
         $this->erro_campo = "tre10_cadenderbairrocadenderrua";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(trim((string) $this->tre10_ordem)!="" || isset($GLOBALS["HTTP_POST_VARS"]["tre10_ordem"])){ 
       $sql  .= $virgula." tre10_ordem = $this->tre10_ordem ";
       $virgula = ",";
       if(trim((string) $this->tre10_ordem) == null ){ 
         $this->erro_sql = " Campo Ordem nao Informado.";
         $this->erro_campo = "tre10_ordem";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       ...
       if($this->numrows>0){
         ...
         for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
           ...
           if(isset($GLOBALS["HTTP_POST_VARS"]["tre10_sequencial"]) || $this->tre10_sequencial != "")
             $resac = db_query("insert into db_acount values($acount,3607,20106,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'tre10_sequencial'))."','$this->tre10_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if(isset($GLOBALS["HTTP_POST_VARS"]["tre10_linhatransporteitinerario"]) || $this->tre10_linhatransporteitinerario != "")
             $resac = db_query("insert into db_acount values($acount,3607,20107,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'tre10_linhatransporteitinerario'))."','$this->tre10_linhatransporteitinerario',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if(isset($GLOBALS["HTTP_POST_VARS"]["tre10_cadenderbairrocadenderrua"]) || $this->tre10_cadenderbairrocadenderrua != "")
             $resac = db_query("insert into db_acount values($acount,3607,20108,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'tre10_cadenderbairrocadenderrua'))."','$this->tre10_cadenderbairrocadenderrua',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if(isset($GLOBALS["HTTP_POST_VARS"]["tre10_ordem"]) || $this->tre10_ordem != "")
             $resac = db_query("insert into db_acount values($acount,3607,20109,'".AddSlashes(pg_fetch_result($resaco,$conresaco,'tre10_ordem'))."','$this->tre10_ordem',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3607,20106,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'tre10_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,3607,20107,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'tre10_linhatransporteitinerario'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,3607,20108,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'tre10_cadenderbairrocadenderrua'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,3607,20109,'','".AddSlashes(pg_fetch_result($resaco,$iresaco,'tre10_ordem'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
